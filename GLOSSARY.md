...
**Pipeline Selector**: "The three-option `Auto` / `Auto (fast implementation)` / `Manual` native picker emitted at the close of every `sai-explore` crystallization turn — the explicit user act that authorizes the selected delegated route, and, after a clean Auto-fast slice completion while pending slices remain, re-presented as a per-slice authorization gate rather than a one-time authorization for the entire crystallized set."
...
- An **Overview Language** value is supplied by an explicit overview-language flag or selected by the overview-language gate at crystallization (or is the literal `None`), rides in the `Ready to Propose` block, and the selected language is forwarded only through a **Supervised Pipeline** started by the **Pipeline Selector**'s `Auto` option; `None` omits the overview-language flag from the chained design envelope, and the value is never written to any artifact.
- A **Pipeline Selector** closes every crystallization turn and, after a clean Auto-fast slice completion while pending slices remain, is re-presented for the next per-slice authorization; `Auto` starts the **Supervised Pipeline**, `Auto (fast implementation)` starts the fast lane, and `Manual` starts none and leaves later execution to an explicit request.
