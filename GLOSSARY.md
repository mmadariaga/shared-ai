...
**Overview Language**: "The conversation-only, chat-scoped value for a change's overview, supplied by an explicit overview-language flag, selected by the overview-language gate (gate 9) at supervised `sai-explore` **Auto** activation, or set to the literal `None`; it is carried in the `Ready to Propose` block only when knowable at emission, and absence never selects a language implicitly."
...
**Pipeline Selector**: "The three-option `Auto (sai-1 + sai-2)` / `Auto (fast implementation)` / `Manual` native picker emitted at the close of every `sai-explore` crystallization turn — the explicit user act that authorizes the selected delegated route, with the first displayed label mapping to the internal `Auto` route, and, after a clean Auto-fast slice completion while pending slices remain, re-presented as a per-slice authorization gate rather than a one-time authorization for the entire crystallized set."
...
- An **Overview Language** value is supplied by an explicit overview-language flag or selected by the overview-language gate at supervised **Auto** activation (or is the literal `None`), remains conversation-only per crystallized idea or slice set, and the selected language is forwarded only through a **Supervised Pipeline** started by the **Pipeline Selector**'s displayed `Auto (sai-1 + sai-2)` option (internal route `Auto`); `None` omits the overview-language flag from the chained design envelope, and the value is never written to any artifact.
- A **Pipeline Selector** closes every crystallization turn and, after a clean Auto-fast slice completion while pending slices remain, is re-presented for the next per-slice authorization; displayed `Auto (sai-1 + sai-2)` starts the **Supervised Pipeline** through internal route `Auto`, `Auto (fast implementation)` starts the fast lane, and `Manual` starts none and leaves later execution to an explicit request.
...
- **Overview State vs Overview Language** — both are "overview"-prefixed pipeline values: `overview.state` is the persisted `.openspec.yaml` materialization key, while **Overview Language** is the chat-scoped, conversation-only value supplied by an explicit overview-language flag, selected at gate 9 during supervised **Auto** activation, or set to the literal `None`. **Resolution:** **Overview State** names the persisted `overview.state` key (`unmaterialized` / `materializing` / `failed` / `current` / `stale`); **Overview Language** names the selected value that forwards only through a **Pipeline Selector** `Auto` run, with `None` omitting the overview-language flag from the chained design envelope; the qualified terms are always used and the two never share a surface.
