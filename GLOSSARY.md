...
- **Idea Progress List vs Progress Plan** — both render as a live checkbox task list, so a bare "task list" or "progress list" is ambiguous about which species is meant: the **Progress Plan** on a routed coordinator's **Native Task Panel**, the **Idea Progress List** on the panel where one exists and as plain in-conversation text elsewhere. **Resolution:** **Progress Plan** names the static, adapter-declared plan of one routed command invocation, marked only from worker progress events; **Idea Progress List** names the `sai-explore` chat-scoped list tracking the explored idea, grown and marked only from in-session evidence; the qualified terms are always used.
