...
**File Manifest**: "The flat `interfaces.md` subsection under **Target State** that lists every file the change creates, modifies, deletes, or renames — path-sorted, git-status-style, with step attribution — derived by a deterministic net fold over the per-step `**Files Affected**` entries of `tasks.md`."
...
**Idea Progress List**: "The chat-scoped, in-conversation-only list in a `sai-explore` session that tracks the explored idea rather than the command's own steps, with research, slice-crystallization, and reviewed sai-1 / sai-2 items grown and marked only from in-session evidence."
...
- An **Idea Progress List** belongs to one `sai-explore` chat and is grown and marked only from in-session evidence; it is never written to a file or derived from repository state.
...
- **"Files Affected" vs "File Manifest"** — the per-step `tasks.md` field and the aggregated `interfaces.md` subsection both describe the change's file surface. **Resolution:** **Files Affected** names the per-step field on `tasks.md` (authoritative per step); **File Manifest** names the aggregated, net-folded subsection on `interfaces.md` (derivative target-state view) — the two are never synonyms, and `tasks.md` remains the authority.
...
- **Idea Progress List vs Progress Plan** — both render as a live checkbox task list in conversation, so a bare "task list" or "progress list" is ambiguous about which species is meant. **Resolution:** **Progress Plan** names the static, adapter-declared plan of one routed command invocation, marked only from worker progress events; **Idea Progress List** names the `sai-explore` chat-scoped list tracking the explored idea, grown and marked only from in-session evidence; the qualified terms are always used.
