...
**Next Pointer**: "The pointer-only transition output (`{follow, hint}`) naming the step file the caller must fetch next, under a fixed minimal template with path interpolation."
*Avoid*: active step line, instruction block, rendered step
...
**Overview Language**: "The conversation-only, chat-scoped value for a change's overview, supplied by an explicit overview-language flag, selected by the overview-language gate (gate 9) at supervised `sai-explore` **Plan (unattended)** activation, or set to the literal `None`; it is carried in the `Ready to Propose` block only when knowable at emission, and absence never selects a language implicitly."
...
**Phase Adapter**: "The per-routed-phase field set consumed by the shared command runner — initial worker envelope, dispatch and continuation operations, permitted nonterminal extensions and handlers, replacement-reconstruction fields, `terminal_navigation`, and optional static `progress_plan`, `step_machine`, and `recovery_policy` — that supplies phase-specific behavior without reimplementing lifecycle mechanics. `progress_plan` controls visual task-list rendering; `step_machine` independently controls just-in-time worker instruction routing."
...
**Pipeline Selector**: "The three-option `Plan - Unattended` / `Direct Build - Unattended` / `Manual` native picker emitted at the close of every `sai-explore` crystallization turn — the explicit user act that authorizes the selected delegated route, and, after a clean Direct Build (unattended) slice completion while pending slices remain, re-presented as a per-slice authorization gate rather than a one-time authorization for the entire crystallized set."
...
**Progress Event**: "An additive, non-terminal lifecycle event a routed planning worker returns to report completed plan steps, carrying exactly `step_ids: string[]` and `changed_files`, with no time field, structurally modelled on the design notice."
*Avoid*: progress payload, status update, tick, notice variant

**Validated At**: "The validator-observed ISO-8601 instant `validated_at` — local wall-clock time with its numeric UTC offset — emitted as an additive display-only sidecar on a valid validator verdict, recording when the pipeline saw the result."
...
- A **Progress Step** of a routed phase progress task list carries exactly one **Milestone Stamp** once it renders `completed`, and none before; the stamp's value is the **Validated At** sidecar of the verdict that marked it.
...
- An **Overview Language** value is supplied by an explicit overview-language flag or selected by the overview-language gate at supervised **Plan (unattended)** activation (or is the literal `None`), remains conversation-only per crystallized idea or slice set, and the selected language is forwarded only through a **Supervised Pipeline** started by the **Pipeline Selector**'s `Plan (unattended)` option; `None` omits the overview-language flag from the chained design envelope, and the value is never written to any artifact.
- A **Pipeline Selector** closes every crystallization turn and, after a clean Direct Build (unattended) slice completion while pending slices remain, is re-presented for the next per-slice authorization; `Plan (unattended)` starts the **Supervised Pipeline**, `Direct Build (unattended)` starts the direct implementation lane, and `Manual` starts none and leaves later execution to an explicit request.
...
- **Milestone Stamp vs Validated At** — both name the same instant on a stamped run, so a bare "time" is ambiguous between the verdict value and its rendering. **Resolution:** **Validated At** is the validator-observed offset-bearing ISO-8601 `validated_at` sidecar on a valid verdict; **Milestone Stamp** is that value's own HH:MM field, rendered on a `completed` step of a progress task list. The stamp is read straight off the observation time and from nothing else — the coordinator issues no wall-clock call and resolves no timezone — so the two are never independent readings that could disagree.
...
- **Overview State vs Overview Language** — both are "overview"-prefixed pipeline values: `overview.state` is the persisted `.openspec.yaml` materialization key, while **Overview Language** is the chat-scoped, conversation-only value supplied by an explicit overview-language flag, selected at gate 9 during supervised **Plan (unattended)** activation, or set to the literal `None`. **Resolution:** **Overview State** names the persisted `overview.state` key (`unmaterialized` / `materializing` / `failed` / `current` / `stale`); **Overview Language** names the selected value that forwards only through a **Pipeline Selector** `Plan (unattended)` run, with `None` omitting the overview-language flag from the chained design envelope; the qualified terms are always used and the two never share a surface.
