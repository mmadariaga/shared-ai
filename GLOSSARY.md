...
**Pipeline Selector**: "The three-option `Plan (unattended)` / `Build (unattended)` / `Manual` native picker emitted at the close of every `sai-explore` crystallization turn — the explicit user act that authorizes the selected delegated route, and, after a clean Build (unattended) slice completion while pending slices remain, re-presented as a per-slice authorization gate rather than a one-time authorization for the entire crystallized set."
...
- A **Pipeline Selector** closes every crystallization turn and, after a clean Build (unattended) slice completion while pending slices remain, is re-presented for the next per-slice authorization; `Plan (unattended)` starts the **Supervised Pipeline**, `Build (unattended)` starts the direct implementation lane, and `Manual` starts none and leaves later execution to an explicit request.
