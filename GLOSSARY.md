...
**Overview Language**: "The conversation-only, invocation-scoped value for a change's overview, supplied by an explicit overview-language flag, selected by the overview-language gate (gate 9) at `sai-explore` crystallization, or set to the literal `None`; it is carried in the `Ready to Propose` block as a durable reminder, and absence never selects a language implicitly."
...
- An **Overview Language** value is supplied by an explicit overview-language flag or selected by the overview-language gate at crystallization (or is the literal `None`), rides in the `Ready to Propose` block, and the selected language is forwarded only through a **Supervised Pipeline** started by the **Pipeline Selector**'s `Plan (unattended)` option; `None` omits the overview-language flag from the chained design envelope, and the value is never written to any artifact.
