...
Also supports **GitHub Copilot** (VS Code in editor window, not agents window) natively — a good fit if you already have a Copilot subscription and prefer to stay inside VS Code without an extra tool. Model selection is more limited than opencode, but the full pipeline runs end to end.
