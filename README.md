...
| `/sai-merge` | Merges a local branch into the current branch with contextual conflict decisions: it explains each branch's objective, compares complete alternatives, and offers a safe synthesis only when both objectives can be preserved without duplicated ownership or sources of truth. Obvious conflicts stay lightweight; semantic ambiguity requires an informed human decision, including under `--fast-track` (which bypasses only the scope gate). It retains the unconditional post-merge ADR/DDR collision pass, verification loop (up to 3 rounds), coordinator-only mutations, and explicit final-commit authorization. |
