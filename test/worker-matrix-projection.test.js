...
const NINE_WORKER_NAMES = [...Object.values(WORKER_NAME), ...Object.keys(APPLY_WORKER_NAME)];
...
test('matrix expansion yields exactly nine worker bindings and nine managed agents per harness with no retired per-phase sources', () => {
  ...
  for (const harness of ['claude', 'opencode']) {
    ...
    const isMatrixAgent = projection =>
      ...
      NINE_WORKER_NAMES.includes(path.basename(projection.destinationPath, '.md'));
    const bindingProjections = active.filter(isMatrixBinding);
    assert.equal(bindingProjections.length, 9,
      `${harness} should project exactly nine active worker bindings`);
    ...
    assert.deepEqual(
      ...
      `${harness} phase bindings should cover exactly the seven canonical phases`
    );
    ...
    assert.equal(agentProjections.length, 9,
      `${harness} should project exactly nine active managed agents`);
    assert.deepEqual(
      agentProjections.map(projection => path.basename(projection.destinationPath, '.md')).sort(),
      [...NINE_WORKER_NAMES].sort(),
    ...
    if (harness === 'claude') {
      assert.equal(['budget', 'executor', 'explore'].every(name => allAgentNames.includes(name)), false,
        'claude must keep its nine matrix agents only');
    } else {
...
test('missing, duplicated, or misassigned matrix entries fail at manifest expansion naming the harness and phase', () => {
  ...
  assert.equal(matrix.entries.length, 9,
    'the canonical entry set should carry nine entries');
...
test('the canonical manifest declares the nine-entry matrix with RED then GREEN apply identities', () => {
  const manifest = loadInstallManifest(REPO_ROOT);
  ...
  assert.equal(entries.length, 9,
    'the canonical manifest should declare nine matrix entries');
  assert.deepEqual(entries.slice(0, 7).map(entry => entry.phase), [...PHASE_ORDER],
    'the seven canonical phase identities should precede the apply entries unchanged');
  assert.deepEqual(entries.slice(0, 7).map(entry => entry.workerName), [...WORKER_NAMES],
    'the seven canonical worker identities should be unchanged');
  const applyEntries = entries.slice(7);
  assert.deepEqual(applyEntries.map(entry => entry.workerName),
    ['sai-4-red-worker', 'sai-4-green-worker'],
    'the apply identities should follow the seven phases in RED then GREEN order');
...
test('the harness binding validators require the nine-worker managed roster', () => {
  assert.deepEqual([...matrixWorkerRoster('claude')].sort(), [...NINE_WORKER_NAMES].sort(),
    'the Claude roster derived from the canonical manifest should contain exactly the nine managed workers');
  assert.doesNotThrow(() => validateClaudeWorkerBindings(),
    'validateClaudeWorkerBindings should accept the nine-worker Claude roster');
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sai-nine-roster-'));
  try {
    ...
    const names = (Array.isArray(roster) ? roster : Object.keys(roster || {}))
      ...
      .sort();
    assert.deepEqual(names, [...NINE_WORKER_NAMES].sort(),
      'the opencode binding roster should contain exactly the nine managed workers');
  } finally {
    fs.rmSync(tmpDir, { recursive: true, force: true });
  }
});

test('install, doctor, and uninstall derive the nine-worker roster and retired destinations without projection-ID collisions', async () => {
  const manifest = loadInstallManifest(REPO_ROOT);
  const projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'sai-matrix-nine-parity-'));
  const claudeBase = path.join(projectRoot, 'claude');
  const opencodeBase = path.join(projectRoot, 'opencode');
  fs.mkdirSync(path.join(projectRoot, 'openspec'), { recursive: true });
  fs.writeFileSync(path.join(projectRoot, 'openspec', 'config.yaml'), 'schema: sai-workflow\n');
  ...
  try {
    for (const [harness, base] of [['claude', claudeBase], ['opencode', opencodeBase]]) {
      ...
      assert.equal(bindings.length, 9, `${harness} should install nine worker bindings`);
      const agents = active.filter(projection =>
        projection.destinationPath.startsWith(roots.agents) &&
        NINE_WORKER_NAMES.includes(path.basename(projection.destinationPath, '.md')));
      assert.equal(agents.length, 9, `${harness} should install nine managed agents`);
    ...
    assert.equal(code, 0, 'doctor should run cleanly on the nine-worker fixture');
    for (const [harness, base] of [['claude', claudeBase], ['opencode', opencodeBase]]) {
      const entries = harness === 'claude' ? enumerateClaude(base) : enumerateOpencode(base);
      const basenames = entries
        .filter(entry => entry.assetType === 'claude-managed-agent')
        .map(entry => path.basename(entry.dest, '.md'));
      assert.ok(NINE_WORKER_NAMES.every(name => basenames.includes(name)),
        `${harness} uninstall should enumerate all nine managed workers`);
    }
  } finally {
    fs.rmSync(projectRoot, { recursive: true, force: true });
  }
