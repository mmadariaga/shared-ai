...
const TEN_WORKER_NAMES = [...Object.values(WORKER_NAME), ...Object.keys(APPLY_WORKER_NAME)];
...
test('matrix expansion yields exactly ten worker bindings and ten managed agents per harness with no retired per-phase sources', () => {
  ...
  for (const harness of ['claude', 'opencode']) {
    ...
    const isMatrixAgent = projection =>
      ...
      TEN_WORKER_NAMES.includes(path.basename(projection.destinationPath, '.md'));
    const bindingProjections = active.filter(isMatrixBinding);
    assert.equal(bindingProjections.length, 10,
      `${harness} should project exactly ten active worker bindings`);
    ...
    assert.equal(agentProjections.length, 10,
      `${harness} should project exactly ten active managed agents`);
    assert.deepEqual(
      agentProjections.map(projection => path.basename(projection.destinationPath, '.md')).sort(),
      [...TEN_WORKER_NAMES].sort(),
      `${harness} managed agents should be exactly the ten worker identities`
    );
    ...
    if (harness === 'claude') {
      assert.equal(['budget', 'executor', 'explore'].every(name => allAgentNames.includes(name)), false,
        'claude must keep its ten matrix agents only');
    } else {
...
test('missing, duplicated, or misassigned matrix entries fail at manifest expansion naming the harness and phase', () => {
  ...
  assert.equal(matrix.entries.length, 10,
    'the canonical entry set should carry ten entries');
...
test('the canonical manifest declares the ten-entry matrix with RED then GREEN apply identities', () => {
  const manifest = loadInstallManifest(REPO_ROOT);
  ...
  assert.equal(entries.length, 10,
    'the canonical manifest should declare ten matrix entries');
  assert.deepEqual(entries.slice(0, 8).map(entry => entry.phase), [...PHASE_ORDER],
    'the eight canonical phase identities should precede the apply entries unchanged');
  assert.deepEqual(entries.slice(0, 8).map(entry => entry.workerName), [...WORKER_NAMES],
    'the eight canonical worker identities should be unchanged');
  const applyEntries = entries.slice(8);
  assert.deepEqual(applyEntries.map(entry => entry.workerName),
    ['sai-4-red-worker', 'sai-4-green-worker'],
    'the apply identities should follow the eight phases in RED then GREEN order');
...
test('the harness binding validators require the ten-worker managed roster', () => {
  assert.deepEqual([...matrixWorkerRoster('claude')].sort(), [...TEN_WORKER_NAMES].sort(),
    'the Claude roster derived from the canonical manifest should contain exactly the ten managed workers');
  assert.doesNotThrow(() => validateClaudeWorkerBindings(),
    'validateClaudeWorkerBindings should accept the ten-worker Claude roster');
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sai-ten-roster-'));
  try {
    ...
    const names = (Array.isArray(roster) ? roster : Object.keys(roster || {}))
      ...
      .sort();
    assert.deepEqual(names, [...TEN_WORKER_NAMES].sort(),
      'the opencode binding roster should contain exactly the ten managed workers');
  } finally {
    fs.rmSync(tmpDir, { recursive: true, force: true });
  }
});

test('install, doctor, and uninstall derive the ten-worker roster and retired destinations without projection-ID collisions', async () => {
  const manifest = loadInstallManifest(REPO_ROOT);
  const projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'sai-matrix-ten-parity-'));
  const claudeBase = path.join(projectRoot, 'claude');
  const opencodeBase = path.join(projectRoot, 'opencode');
  fs.mkdirSync(path.join(projectRoot, 'openspec'), { recursive: true });
  fs.writeFileSync(path.join(projectRoot, 'openspec', 'config.yaml'), 'schema: sai-workflow\n');
  ...
  try {
    for (const [harness, base] of [['claude', claudeBase], ['opencode', opencodeBase]]) {
      ...
      assert.equal(bindings.length, 10, `${harness} should install ten worker bindings`);
      const agents = active.filter(projection =>
        projection.destinationPath.startsWith(roots.agents) &&
        TEN_WORKER_NAMES.includes(path.basename(projection.destinationPath, '.md')));
      assert.equal(agents.length, 10, `${harness} should install ten managed agents`);
    ...
    assert.equal(code, 0, 'doctor should run cleanly on the ten-worker fixture');
    for (const [harness, base] of [['claude', claudeBase], ['opencode', opencodeBase]]) {
      const entries = harness === 'claude' ? enumerateClaude(base) : enumerateOpencode(base);
      const basenames = entries
        .filter(entry => entry.assetType === 'claude-managed-agent')
        .map(entry => path.basename(entry.dest, '.md'));
      assert.ok(TEN_WORKER_NAMES.every(name => basenames.includes(name)),
        `${harness} uninstall should enumerate all ten managed workers`);
    }
  } finally {
    fs.rmSync(projectRoot, { recursive: true, force: true });
  }
