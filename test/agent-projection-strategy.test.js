...
test('the manifest declares 22 matrix tunable-seed managed agent projections', () => {
  ...
  for (const harness of ['claude', 'opencode']) {
    ...
    assert.equal(agentProjections.length, 11,
      `the manifest should declare exactly 11 matrix agent projections for ${harness}`);
    ...
    assert.deepEqual(
      ...
      `${harness} matrix agent projections should cover exactly the eleven worker identities`);
  ...
  assert.equal(allAgentProjections.length, 22,
    'the manifest should declare 22 matrix managed agent projections across both harnesses');
});

test('the manifest declares exactly 28 tunable-seed agents projections, exactly 14 targeting Claude', () => {
  ...
  assert.equal(tunableAgentRules.length, 28,
    'the manifest should declare exactly 28 tunable-seed agent-class projections');
  assert.equal(
    tunableAgentRules.filter(projection => projection.harnesses.includes('claude')).length, 14,
    'exactly 14 tunable-seed agent-class projections should target Claude');
  assert.equal(
    tunableAgentRules.filter(projection => projection.harnesses.includes('opencode')).length, 14,
    'exactly 14 tunable-seed agent-class projections should target opencode');
});
...
test('no managed agent projection declares owned-copy', () => {
  ...
  assert.equal(matrixAgentRules.length, 22,
    'the manifest should declare 22 matrix agent-class projections (eleven per harness)');
  assert.equal(
    matrixAgentRules.filter(projection => projection.harnesses.includes('claude')).length, 11,
    'claude should declare exactly eleven matrix agent projections');
  assert.equal(
    matrixAgentRules.filter(projection => projection.harnesses.includes('opencode')).length, 11,
    'opencode should declare exactly eleven matrix agent projections');
