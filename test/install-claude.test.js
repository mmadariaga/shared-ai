...
test('installClaude copies five Claude-specific skills', () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sai-claude-'));
  installClaude(tmpDir);
  ...
  assert.ok(!fs.existsSync(path.join(tmpDir, 'skills', 'budget', 'SKILL.md')), 'skills/budget/SKILL.md must NOT be in Claude install (Opencode-only)');
  fs.rmSync(tmpDir, { recursive: true, force: true });
});
