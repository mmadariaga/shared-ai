...
const PERFORMANCE_STEP_MAP = {
  'resolve-performance-scope': null,
...
test('step-gated: the performance coordinator declares a static step_pointer_map over exactly the five plan ids', () => {
  const coordinator = artifact('sai/commands/performance/coordinator.md');
  ...
  assert.deepEqual(rows.map(([id]) => id), PERFORMANCE_PLAN_STEPS.map(([id]) => id),
    'the map should cover every declared plan id exactly once, in plan order');
  assert.equal(rows[0][1], 'none', 'resolve-performance-scope stays fileless');
  for (const [id] of PERFORMANCE_PLAN_STEPS.slice(1)) {
    const expected = `\`@${PERFORMANCE_STEP_MAP[id]}\``;
...
test('step-gated: the performance worker loads steps/common.md at dispatch and executes only the active step', () => {
  const worker = artifact('sai/commands/performance/worker.md');
...
test('step-gated: the carved step library exists beside the untouched monolith', () => {
  ...
  assert.match(artifact('sai/commands/performance/steps/common.md'), /`resolve-performance-scope` has no step file of its own/,
    'common.md should record that resolve-performance-scope is fileless');
});
