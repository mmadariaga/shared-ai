...
test('discovered coordinators have registered step machines and fetch stage-machine.md', () => {
  ...
  // Must discover exactly spec, design, implement, performance, review, and security
  const names = coordinators.map(c => c.name).sort();
  const expectedPhases = ['design', 'implement', 'performance', 'review', 'security', 'spec'];
  assert.deepEqual(names, expectedPhases, 'must discover exactly spec, design, implement, performance, review, and security');
...
test('coordinator progress plans match their registered machines (source-driven)', () => {
  ...
  for (const coord of coordinators) {
    ...
    } else if (coord.name === 'implement' || coord.name === 'performance' || coord.name === 'review' || coord.name === 'security') {
