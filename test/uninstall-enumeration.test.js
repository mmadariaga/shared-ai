...
test('install and uninstall inventories are exact and deterministic for every supported harness', () => {
  ...
  for (const { harness, install, enumerate } of cases) {
    ...
    try {
      ...
      assert.equal(activeWorkerBindings.length, 15,
        `${harness} should project exactly fifteen active worker bindings`);
      const activeWorkers = normalizedActive
        .filter(destination => /^agents\/sai-(?:\d-.*|commit|archive|backfill|merge|autofast-implement|autofast-hands)-worker\.md$/.test(destination));
      assert.equal(activeWorkers.length, 15,
