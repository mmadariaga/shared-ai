...
test('step-gated: the accessibility coordinator declares a static step_pointer_map over exactly the five plan ids', () => {
  const coordinator = artifact('sai/commands/accessibility/coordinator.md');
  ...
  assert.deepEqual(rows.map(([id]) => id), ACCESSIBILITY_PLAN_STEPS.map(([id]) => id),
    'the map should cover every declared plan id exactly once, in plan order');
  assert.equal(rows[0][1], 'none', 'resolve-accessibility-scope stays fileless');
  for (const [id] of ACCESSIBILITY_PLAN_STEPS.slice(1)) {
    const expected = `\`@${ACCESSIBILITY_STEP_MAP[id]}\``;
