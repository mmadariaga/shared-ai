...
test('installOpencode projects the adapter idea-list render glue and resolves it from sai-explore', () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sai-opencode-idea-list-adapter-'));
...
test('Step 2 Opencode installation preserves the shared selector contract without harness-specific selector semantics', () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sai-opencode-selector-contract-'));
  try {
    installOpencode(tmpDir);
    ...
    const panel = fs.readFileSync(path.join(tmpDir, 'sai', 'adapters', 'opencode', 'panel-render.md'), 'utf8');
    const ideaList = fs.readFileSync(path.join(tmpDir, 'sai', 'adapters', 'opencode', 'idea-list-render.md'), 'utf8');
...
test('Step 3 binding roster validation yields exactly the fifteen managed workers', () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sai-opencode-roster-'));
  const WORKERS = [...CURRENT_CENSUS, 'sai-4-red-worker', 'sai-4-green-worker', 'sai-direct-build-worker', 'sai-review-fix-worker'];
  try {
    installOpencode(tmpDir);
    const bindingsDir = path.join(tmpDir, 'sai', 'orchestration', 'workers', 'bindings');
    ...
    assert.deepEqual(names, [...WORKERS].sort(),
      'specs/opencode-agent-census/spec.md: the binding roster must contain exactly the fifteen managed workers with no extra or missing worker');
  } finally {
    fs.rmSync(tmpDir, { recursive: true, force: true });
  }
});

test('Step 3 binding files declare exactly the fifteen initial worker dispatches', () => {
...
test('opencode installer consumes exactly the fifteen matrix worker bindings and agents', () => {
  ...
  try {
    ...
    assert.equal(bindingNames.length, 10, 'opencode should project exactly ten phase worker bindings');
    ...
    assert.ok(ideaList, 'opencode should project the adapter idea-list render source');
    assert.equal(
...
test('Step 3 the opencode neutral inventory is equivalent to Claude and differs only at the boot adapter seam', () => {
  const repoRoot = path.join(__dirname, '..');
  const manifest = loadInstallManifest(repoRoot);
  const bases = {};
  for (const harness of ['claude', 'opencode']) {
    bases[harness] = fs.mkdtempSync(path.join(os.tmpdir(), `sai-step3-opencode-neutral-${harness}-`));
  }
  ...
  try {
    ...
    assert.ok(Object.keys(opencode).length > 0, 'the opencode neutral inventory should be non-empty');
    assert.deepEqual(opencode, claude,
      'the opencode neutral inventory should equal the Claude neutral inventory');
    for (const source of Object.keys(opencode)) {
