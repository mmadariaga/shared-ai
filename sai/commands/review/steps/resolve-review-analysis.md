...
Active step: resolve-review-analysis. Perform review passes 1–10 plus the dedicated Resilience pass against the scoped diff, then report the `resolve-review-analysis` progress event per the worker contract.
...
2. **Correctness & Bugs** — Logic errors, off-by-one, null/undefined handling, race conditions, incorrect API usage, broken edge cases. Retries, timeouts, circuit-breaker, idempotency and fallback belong to Resilience; do not duplicate them here.
...
4. **Performance (triage only — DO NOT deep audit)** — Detect whether the diff touches **performance surface**:
     ...
     Your job here is **not** to run EXPLAIN, profile, or measure CWV. Only flag *surface touched: yes/no* and list the specific files. If yes, recommend `/sai-7-performance` in the report. Timeout and retry concerns belong to Resilience; do not duplicate them here. Do not raise individual performance findings unless they are blatant (e.g. nested loop on a known-large collection, `SELECT *` inside a per-row loop, render-blocking `<script>` without `defer`) — those go as High or Critical with a note that `/sai-7-performance` will cover the rest.
...
11. **Resilience (dedicated pass — single owner for retries, timeouts, circuit-breaker, idempotency and fallback)** — Detect only on **resilience surface**: external I/O, retryable handlers, consumers and queues, and timeout boundaries; UI files without I/O are exempt. Flag unbounded retries, missing timeouts on critical I/O, missing idempotency where retry, redelivery or double submit is possible, and absent fallback where impact warrants it. Idempotency is required only when retry, redelivery or double submit is possible — without retry its absence is not a finding. Critical only for cascade, loss or duplication risk with real impact (infinite retry on payments, consumer without idempotency with real retry, critical call without timeout); missing timeout on a non-critical path or absent fallback without impact is High or Medium. Never require a retry, timeout, circuit-breaker, idempotency or fallback pattern that does not already exist in the repo — when no convention exists, at most Question or Low, never Critical. Docs-only, comments, CSS without I/O, or renames are recorded as no surface with no findings, not as an empty pass. Correctness and Performance-triage do not duplicate these points.
