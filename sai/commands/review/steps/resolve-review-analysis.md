...
11. **Resilience (dedicated pass — single owner for retries, timeouts, circuit-breaker, idempotency and fallback)** — Detect only on **resilience surface**: external I/O, retryable handlers, consumers and queues, and timeout boundaries; UI files without I/O are exempt. Flag unbounded retries, missing timeouts on critical I/O, missing idempotency where retry, redelivery or double submit is possible, and absent fallback where impact warrants it. Idempotency is required only when retry, redelivery or double submit is possible — without retry its absence is not a finding. Docs-only, comments, CSS without I/O, or renames are recorded as no surface with no findings, not as an empty pass. When the repo has no existing resilience pattern for the case, cap at Question/Low — never demand a new pattern as High/Critical. This pass owns retry/timeout/circuit-breaker/idempotency/fallback exclusively; Correctness and Performance-triage do not duplicate them here. Severity: Critical only for cascade/outage, data loss, or duplicate side-effects with concrete impact; otherwise High/Medium/Low by blast radius.
