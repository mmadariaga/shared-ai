...
Execute review passes 1-11 from the active `resolve-review-analysis` step file. Keep all research read-only and use `budget-explorer`. Then execute Pass 12 only under its activation gate, following the active `resolve-mutation-analysis` step file. Run the declared deterministic mutation tool with its checked-in project configuration, scoped to the eligible diff files, and parse its real report. If no deterministic tool is declared, or execution/report parsing fails, record that fact and continue without mutation findings; never choose or simulate mutations in the worker.

Write and verify only `openspec/changes/{change-name}/review.md` outside temporary bounded Pass 12 mutations. `changed_files` contains `review.md` plus only production files whose revert failed or whose outcome was unaccounted; exclude cleanly reverted targets.

Before returning `completed`, verify `review.md` exists, is non-empty, and contains findings with severity-prefixed identifiers (`C1`/`H1`/`M1`/`L1`/`Q1`), severity roll-up, coverage, Pass 12 outcomes, all three audit recommendations, and a closing `Summary:` tally line whose counts match the report's findings (mutation findings folded at their remapped severities). The worker-authored summary contains severity counts, top three Critical findings when present, report path, parent-branch statement, and the complete verbatim `## Recommended Audits` block. Return no report contents.
