...
Execute review passes 1-10 plus the dedicated Resilience pass from the active `resolve-review-analysis` step file. Keep all research read-only and use `budget-explorer`. Then execute Pass 11 only under its activation gate, following the active `resolve-mutation-analysis` step file. Run the declared deterministic mutation tool with its checked-in project configuration, scoped to the eligible diff files, and parse its real report. If no deterministic tool is declared, or execution/report parsing fails, record that fact and continue without mutation findings; never choose or simulate mutations in the worker.
