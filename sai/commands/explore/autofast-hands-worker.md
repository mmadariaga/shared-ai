...
5. **Commit-message authoring** — from the staged state only, author one
   ...
   is staged.
6. **Local commit** — execute the commit with the HEREDOC form
...
duplicate-free `changed_files`. On success the summary reports, in order:
the written artifact paths, the sync result, the archive destination
directory, the staged path count, and the commit subject line. Return
