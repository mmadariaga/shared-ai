...
> Cold-build source for the index-maintenance hook in `sai/commands/implement/steps/decision-record-index.md`. This file is a **project-agnostic section skeleton** — it carries NO baked-in domain-unit or category snapshot. The cold build derives the `### <domain unit reference>` subsection names under `## By <domain unit>` and the `### <category>` subsection names under `## Cross-cutting categories` from the DDRs' own content at cold-build time. A consumer project's cold build therefore produces that project's own domain-unit/category subsections, never another project's snapshot.
