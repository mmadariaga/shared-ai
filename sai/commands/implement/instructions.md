...
- **Audit-derived step interface contracts**: Appended audit-derived steps in `implementation.md` MUST have a corresponding `## Step N:` contract section in `interfaces.md` if and only if the step introduces a modified interface or testable assertion. Assertions in audit contracts MUST anchor exclusively to requirements that already exist in `specs/**` — audit-derived steps cannot establish new acceptance criteria, only assert against existing ones. Test assertions are governed by the testability rule at instructions.md:164: audit steps with testable code carry a RED block; non-testable audit steps (internal refactors, config changes, scaffolding) omit RED entirely, regardless of whether their findings violated an existing requirement. Steps introducing neither interface nor assertion carry no contract section (following the omission rule in `@sai/policies/step-contract-format.md`).
