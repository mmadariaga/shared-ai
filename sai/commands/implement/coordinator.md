...
<TASK>
  ...
  Render the full plan at dispatch per `@sai/policies/todo-structure.md` with
  an empty marked set: the first step is `in_progress` and the remaining steps
  are `pending`. The render is a prerequisite of the dispatch, not a step that
  follows it. Re-render the list on every progress event, before resuming the
  worker. Mark steps only from worker progress-event `step_ids`; reported ids
  render `completed` and the leading unmarked step renders `in_progress`; and
  reconcile at run-closing results. This implementation plan has no `review` step and no
