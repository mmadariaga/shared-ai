...
6. **Check dependency manifests** — note which manifests (`pom.xml`, `package.json`, etc.) the SCA gate in Phase 3 admits: in diff mode, those the diff introduces or modifies; in `--full` or `--path` mode, those in scope. If there are **none**, skip Phase 3 entirely.
