...
Write and verify only `openspec/changes/{change-name}/security.md`. Before returning `completed`, verify it exists, is non-empty, and contains a concise executive summary, concrete evidence-backed findings each with a severity-prefixed identifier (`C1`/`H1`/`M1`/`L1`), severity counts, a closing `Summary: Critical=<n> High=<n> Medium=<n> Low=<n>` tally line whose counts match the report's findings, acknowledged trade-offs when applicable, and applicable SCA evidence. The worker-authored summary contains severity counts, up to three Critical/High findings when present, the report path, and the selected parent branch. `changed_files` contains only `openspec/changes/{change-name}/security.md`. Return no report contents.
