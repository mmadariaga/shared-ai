...
condition check, diff-source selection, diff computation (`git diff`,
`git diff --staged`, `git diff HEAD`, `git ls-files --others
--exclude-standard` — read-only git surface only) and its summary, optional
intent capture, in-memory intent reconciliation and item classification, the
fixed and adaptive interview, generated reconciliation questions, the scope-
drift report, and conflict detection. Everything is read-only: checks,
...
- **Closed-choice asks** return the ordered options exactly: the diff-source
  ...
  plain-text fallback block preserved verbatim for picker-less surfaces; the
  ...
  selections exactly as the instruction's precedence rules define; the
  conflict decision **"Do you want to proceed with these updates, or abort?"**
  with options `proceed (Recommended)` / `abort`; the change-name proposal
  "I'll use `{proposed-name}` as the change name. Is that correct? (yes/no)"
  with `yes` / `no`. Carry the essential state context alongside each ask —
...
- **Open-ended and free-text asks** return an empty `options` array: the
  base-commit request "Provide the base commit SHA:", Question 1 ("What
  problem does this solve?"), Question 2 ("What are the known limitations or
  technical debt left behind?"), every generated reconciliation question, and
...
written."** Carry the scope-drift lines, the conflict report, and the
