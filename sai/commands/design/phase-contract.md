...
include this boolean alongside `step_ids` in the first progress-event emit,
before any completed id has been recorded. For an opted-in run, that event is
`{"step_ids":["prereqs-resolution"],"withOverview":true}`; an unopted run uses
`"withOverview":false`. This is coordinator-owned machine initialization, not a worker payload
or an argument to `spawn`. Later emits carry only `step_ids`: the variant is
immutable once progress starts. A replacement worker uses the surviving machine
