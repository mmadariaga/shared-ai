<TASK>
  ...
  Declare exactly one of the following canonical static plans, in order, with exactly these ids and labels — no omissions, reorders, renames, or additions. The plan follows the raw-token-presence rule above; the coordinator does not inspect or validate the token's value.
