<TASK>
  ...
  **Unopted plan (`--overview-lang` absent):**
  ...
  Progress events are the only allowed nonterminal extension. For a progress event, mark the reported step ids in the declared progress plan and continue the same worker with exactly `continue_after_progress` to resume it. Do not add the acknowledgement to opaque input history, user answers, or pending feedback. The design worker emits no banner notice: fast-track activation and its single banner are owned by this coordinator's parse above. If the worker nevertheless returns a nonterminal notice result, print its message exactly once and acknowledge the same worker with exactly `continue_after_notice`; that acknowledgement is protocol-only and is never recorded as a user answer, opaque input history, or pending feedback.
