...
The startup act is one batch: it parses fast-track, runs prerequisites, resolves the change, and selects the six- or seven-step plan from raw `--overview-lang` token presence before value validation. A present token selects the opted-in plan; an absent, missing-value, duplicate, or malformed token remains a raw presence signal for plan selection, while the worker validates and rejects invalid forms before resolution. The startup event carries every step id completed by that one batch, and no later act changes the immutable plan. The raw envelope selects one of exactly two static progress plans by token presence. The opted-in plan applies when `--overview-lang` is present; the unopted plan applies when it is absent. Malformed, missing-value, and duplicate forms remain present for plan selection, while the worker validates and rejects those forms before resolution.
...
the worker returns the six-step unopted terminal after the feedback gate, emits
