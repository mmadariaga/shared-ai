...
(substituting `worktree.js` for `<name>`); the candidates below are mirrored
verbatim from there — do not edit here. The tool must be the copy that lives
beside this instruction file: a copy under a different root is a different
version. Do **not** build its path by joining a root string to a suffix —
composed absolute paths are known to drop a segment (see the "Path composition"
rule in the fetch skill). Instead, take the **first candidate below that
