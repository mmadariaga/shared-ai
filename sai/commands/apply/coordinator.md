<TASK>
  ...
  Fetch @sai/policies/remember.md and follow it for native picker use.
  Fetch @sai/policies/question-context.md and follow it for the exhausted-budget choice.
  ...
  ## Chained activation
  Under composition, the supervising composition already holds the resolved change name and sets the fast-track signal explicitly true or false. Chained activation runs this card, the runner, and `invocation.md` § Completion only; it does not run the Prerequisite checks, change-picker resolution, or Fast-track parse. Entry is composition segment activation with the composition-built envelope, not a harness boot or wrapper re-entry. On that entry, activate § Session commit authorization before Run-Start Step Projection or any Step dispatch.

  ## Session commit authorization
  `Allow on this session` activates the in-memory `session_commit_authorized` flag per commit-rules § Authorization Scope. In apply it covers exactly two gates: the per-Step commit gate (runner § Step commit gate) and the eligible terminal documentation commit gate (terminal-lifecycle § 4). At apply segment entry, after the standalone Fast-track parse or the chained supervisor's injected signal is known and before Run-Start Step Projection or first Step work, set `session_commit_authorized=true` if and only if fast-track is active. This is invocation-scoped coordinator state, not a worker envelope field; fail safely if the fast-track state is missing or invalid instead of asking for the first commit. An active flag skips only the authorization ask: the visibility report and proposed message still print before every commit, and the GREEN-conflict STOP (an unpassable GREEN that recovery cannot correct) still halts the run. It does not authorize pushes, branch changes, unrelated files, unresolved-conflict stops, or separate safe-operations confirmations.
  ...
  **Exhausted-Step choice.** When either the worker or coordinator budget is exhausted, and no `unrecoverable: true` veto or safe-operations denial applies, stop before checkbox marking, commit, or Step advance. Read both tallies, the exhausted budget, and all prior cycles from the recovery-ledger response. Present this two-option decision through the active harness's native picker, following `@sai/policies/question-context.md`:

  - **Preceding text** — include a visible `Terminology` section defining `Step` as one numbered unit in the `implementation.md` plan, `recovery budgets` as the limits of three worker attempts and three coordinator attempts for that Step, `new attempt` as one new invocation of the whole blocked Step with both budgets renewed, and `explicit authorization` as the user's retry selection or unequivocal order naming that Step. Then name the active command (`/sai-4-apply` standalone or `/sai-build` chained), the change, the blocked Step, what failed, both spent tallies, and the prior attempt history when present. State that the Step remains unmarked, uncommitted, and unadvanced; the retry option authorizes exactly one new attempt and does not authorize a commit or bypass checks; the manual option stops apply for human correction.
  - **Picker question** — `How do you want to proceed with Step N?`
  - **Options, in order** — `Authorize one fresh attempt` (`authorize-step-retry`); `I will correct it manually` (`manual-correction`).

  Wait for a valid choice. The retry option, or an unequivocal direct order naming this exact Step (for example, `Retry Step N` or `I authorize a fresh attempt for Step N`), is the only authorization. A question such as `Can't you fix it?`, a general request for help, silence, or an off-option answer authorizes nothing; re-present the same choice unchanged when the reply is ambiguous. The manual option closes this apply run with the Step still blocked. Never auto-select either option under `--fast-track`.

  After explicit authorization, re-read the current `implementation.md` Step contract and verification scope. If the plan changed, the Step contract is missing or ambiguous, the correction is not viable, a worker veto applies, or safe-operations denied the action, do not grant a budget; report the exact stop. Otherwise send the shared policy's `authorized-step-retry` event for this Step and require the machine to confirm `retry_grant: granted`. The machine archives the exhausted cycle in `attempt_history` before clearing both budgets; report the retained history and new cycle to the user. Start one new worker-level Step invocation for only this blocked Step, reusing the current worktree and plan because the plan was just verified unchanged. Its ordinary `step-entry` is a re-entry, not another grant, and its first dispatch opens a new no-commit-guard window with a fresh snapshot. Re-select routing and the current plan, then run the normal verification and commit gates. The grant covers the whole Step, not only the original failure. If either fresh budget is exhausted again, stop and ask for a new explicit decision; never chain a grant.
  ...
  **Plan-artifact repair.** An `out-of-scope` cause spends zero worker attempts and hands back to its owner. One exception: when the evidence identifies the exact current-Step verification assertion in `implementation.md` as the defect (for example, it names a later Step's producer that cannot exist yet), you MAY repair that one assertion, spending one coordinator attempt (§ Unblock ladder) and no worker slot. The repair writes only that assertion and preserves Step headings, checkbox semantics and states, plan-level file scope, worker prohibitions, the verification checklist and run boundary, and the Coverage Signature. The repair never runs verification as part of its write. Add `implementation.md` to the union once. Then run the normal § Post-dispatch sequence, whose independent verification stays mandatory; the repair never becomes worker work.
  ...
  - **Budgets.** Each Step has three worker slots and three coordinator attempts, both held by `recovery-ledger@1` and granted by the Step-entry signal only on the Step's first entry (`step_entry: first`; a `re-entry` keeps what was spent; `unidentified` grants nothing). The only fresh-budget exception is the user-authorized `authorized-step-retry` event described above and in `@sai/policies/bounded-recovery.md`; it resets both budgets together and retains the exhausted-cycle history. Before each coordinator attempt, send `{kind: coordinator-attempt, key: [artifact path, concrete point, authorized correction boundary]}` and announce the returned ordinal. A key-less attempt returns `rejected: unresolved cause`; a repeated key returns `rejected: duplicate diagnosis` and hands back the existing diagnosis. Read tallies from the response's `budgets`, never from memory. A delegated corrective dispatch spends a coordinator attempt exactly as a self-edit does, so an exhausted coordinator budget stops the Step even with worker slots left.
