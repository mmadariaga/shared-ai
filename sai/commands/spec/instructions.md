...
> This file is the command-local quality instruction loaded by the routed
> worker's always-active step surface. Lifecycle, progress, pointer, result,
> validation-report, and write-scope declarations belong exclusively to
> `@sai/policies/spec-phase-contract.md`; this file supplies no competing
> declarations.

It adds shared-AI quality behaviors (collaboration style, cost discipline, and
research guidance) without redefining the output template or workflow — those
belong to the OpenSpec skill and the active worker steps.
...
You **do not write code**. Use `@sai/policies/spec-phase-contract.md` as the
sole source for the `SpecWriteSurface`; its allowed deliverables are the only
files this phase may create or modify. Code generation, configuration changes,
and project modifications are the explicit responsibility of downstream
commands (`sai-3-implement`, `sai-4-apply`). Do not perform them during the
spec phase.
...
When invoked as sai-1-spec, use the closed `SpecWriteSurface` from
`@sai/policies/spec-phase-contract.md`: generate only `proposal.md` and
`specs/**/*.md`, with the permitted root `GLOSSARY.md` exception. Do NOT
generate `design.md` or `tasks.md`; those belong to `/sai-2-design` after spec
review.

## Scope Reminder

> The canonical `SpecWriteSurface` is the sole write boundary. Do not write
> project code, configuration, or files outside its declared paths. Code
> generation is the responsibility of a different command.
...
Rule #1 and Rule #2 return ordered metadata-only warning entries through the
`validation_report` extension from
`@sai/policies/spec-phase-contract.md`. The coordinator renders those entries
after the decision summary and before the artifact-feedback gate using the
policy's canonical warning block. The worker never prints or formats the
warning block itself.
