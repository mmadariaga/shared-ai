...
Set `$ARGUMENTS` to the resolved request from the invocation envelope, then write `openspec/changes/{name}/proposal.md` per that skill's template. Generate ONLY `proposal.md` in this step — `specs/**/*.md` are written by the next step; `design.md` and `tasks.md` are never generated in this phase.
