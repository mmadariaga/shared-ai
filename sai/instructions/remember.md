...
- **Closed-choice prompts (universal across harnesses):** whenever you ask the user to pick from a closed set of options (a numbered list, a yes/no question, a commit gate), present the choices through the harness's native option-picker tool when one exists; otherwise fall back to plain text. Per harness:
