...
- **Branch selection**: choose the branch by the absence or presence of `docs/adr/0000-INDEX.md`:
  - **Cold build** (index absent): construct `docs/adr/0000-INDEX.md` from the project-agnostic template at `sai/instructions/_templates/adr-index.md` (read it by exact path — do NOT inline its boilerplate into `implement.md`).
    The template carries only the section skeleton with `## By <domain unit>`. The cold build derives the `<domain unit>` noun deterministically from the ADR-derived domain-unit references and substitutes it into that H2 (for example, `## By command` in this repository, `## By module` in a module-based consumer project): slash command references (e.g. `/sai-*`) map to `command`; `*-module` / `module-*` references map to `module`; HTTP route references map to `endpoint`; package-name references map to `package`; service-name references map to `service`. Choose the noun with the highest ADR count; ties break by this precedence order. If none of these mappings matches, derive the fallback noun from the dominant grouping noun found near the ADR references in the raw context report; if still unresolved, use the literal noun `domain unit`.
  ...
  - **Relationship annotation rules (both branches)**: for every relationship declared by a structured line (or by best-effort prose fallback for hand-written ADRs), annotate ADR entry lines using these exact token forms: `— Pair with NNNN`, `— Refs NNNN`, `— **Amends** NNNN`, `— **Reframes** NNNN`, `— **Reverses** NNNN`, `— Supersedes NNNN`. Do not invent alternate token styles. `*Superseded by [NNNN]*` is reserved for the moved historical entry in the supersede-move rule, not for the superseding ADR entry.
  - **Correction-table population rules (both branches)**: for each relationship of type `amends|supersedes|reverses|reframes`, add one row to `## ADRs that extend or correct prior ones` using `[NNNN_source] | <action> | [NNNN_target]` where `<action>` is exactly `amends`, `supersedes`, `reverses`, or `reframes`. Do not add correction-table rows for `refs` or `pair-with`.
  - **Cold-build supersede move**: move EVERY superseded ADR project-wide (not session-scoped) out of `## By <domain unit>` and `## Cross-cutting categories` into `## Superseded ADRs (historical)`, appending `*Superseded by [NNNN_superseding]*` to each moved entry line and adding correction-table rows for each supersede relation.
  - **Threshold recomputation scope**: the cold build is the SOLE branch that recomputes global cross-cutting thresholds (for example, `>=2` for own subsection and the 8–12 target subsection count). This threshold logic applies only to `## Cross-cutting categories`. The `## By <domain unit>` axis has NO threshold: each distinct domain-unit reference gets its own subsection.
...
- **Index-output invariants** (both branches): write the index in English; use relative Markdown links from the index to each ADR file (`./NNNN-slug.md`, never absolute paths or URLs); preserve each ADR's exact H1 title verbatim (the `{Title}` portion of `# ADR NNNN: {Title}`, without the `# ADR NNNN:` prefix in the entry text); preserve the index's own H1 `# ADR Index` verbatim as the first line of the file across both branches.
