...
- **Branch selection**: choose the branch by the absence or presence of `docs/adr/0000-INDEX.md`:
  - **Cold build** (index absent): construct `docs/adr/0000-INDEX.md` from the project-agnostic template at `sai/compat/_templates/adr-index.md` (read it by exact path — do NOT inline its boilerplate into `implement.md`). The template carries only the section skeleton with `## By <domain unit>`. The cold build derives the `<domain unit>` noun deterministically from the ADR-derived domain-unit references and substitutes it into that H2 (for example, `## By command` in this repository, `## By module` in a module-based consumer project): slash command references (e.g. `/sai-*`) map to `command`; `*-module` / `module-*` references map to `module`; HTTP route references map to `endpoint`; package-name references map to `package`; service-name references map to `service`. Choose the noun with the highest ADR count; ties break by this precedence order. If none of these mappings matches, derive the fallback noun from the dominant grouping noun found near the ADR references in the raw context report; if still unresolved, use the literal noun `domain unit`.
