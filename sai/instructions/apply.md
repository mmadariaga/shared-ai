...
- **Step-N key-integrity guard**: This guard runs only after routing has already selected the two-dispatch flow (part 2 of the routing condition found a `## Step N`). Before dispatching the test-writer, the coordinator SHALL confirm the integer `N` of the current `implementation.md` `## Step N` heading matches exactly one `## Step N` in `interfaces.md`. An **ambiguous** match — several `## Step N` sections for the same integer `N` — is a desync: STOP and surface it to the user, and do NOT inject one of the candidate contracts into the blind writer. **Absence** is not evaluated here: a Step with no matching `## Step N` never reaches the two-dispatch flow, because part 2 of the routing condition already routed it to a single dispatch (see `## Step-Execution Subagent Dispatch`). Neither this guard nor the routing path injects an empty or mismatched contract into a blind writer.
- **Rules**:
