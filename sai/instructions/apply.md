...
Exactly once per completed run, after the Final sweep and `## Learnings Promotion Pass`, derive a fixed terminal set from the current working tree. Include every changed path under `docs/**` and include root `SAI_LEARNINGS.md` only when the current promotion pass wrote it. Evaluate `docs/**` at terminal time with no run-start baseline, so pre-existing uncommitted documentation is eligible. Do not derive this set from a Step, `tasks.md`, an intended add-list, or subagent report field 8.
