...
- **Final sweep:** When all steps in the plan are complete, scan the entire `implementation.md` and verify that every checkbox that should be checked is marked `[x]`. Report any unchecked items to the user before declaring the implementation done. Once the sweep passes, run the single end-of-run promotion pass per `## Learnings Promotion Pass` before the MANDATORY STOP.
...
**Commit gate.** When the pass writes `SAI_LEARNINGS.md`, the coordinator SHALL propose a commit containing that file and no other, and SHALL obtain explicit authorization before running `git commit`. Before proposing it, the coordinator SHALL print a **minimal disclosure**: the path written, and the count of entries added and superseded, broken down by section. The disclosure SHALL additionally list the keys of any pre-seeded entries this run's own execution contradicted — entries superseded because a deviation reported the opposite of what the file recorded. Surfacing them is disclosure only and grants no delete authority; it exists so the human can see which recorded facts the repository has moved past. This disclosure replaces the pre-commit file visibility report for this commit.

The authorization SHALL be a closed-choice `yes` / `no` prompt presented through the harness's native option-picker, with `yes`-only execute semantics: anything other than an explicit `yes` — no, silence, a redirect, or any other reply — is a decline. On decline the coordinator SHALL leave the written file in the working tree, describe it so the user can commit it themselves, and proceed to the MANDATORY STOP without halting or retrying. The promotion commit SHALL stage exactly the path `SAI_LEARNINGS.md`, determined directly by this pass: the coordinator SHALL NOT consult the per-Step intended add-list and SHALL NOT consult subagent report field 8 when staging it.

When no candidate survives the classification, the pass writes nothing, proposes no commit, prints no authorization prompt, and the run proceeds to the MANDATORY STOP.

**A run that halts before the Final sweep promotes nothing.** When a run ends earlier — a GREEN-conflict halt, a user stop, or a decline at a Step's commit gate — the pass SHALL NOT run and `SAI_LEARNINGS.md` SHALL NOT be written for that run. This is not data loss: deviations for every completed Step are already committed to the appendix, and a later run over the same change promotes them from there. The coordinator SHALL NOT attempt a partial or early promotion to salvage facts from a halted run.
...
**One exception — the promotion commit.** This report SHALL NOT be printed for the commit proposed by `## Learnings Promotion Pass`. That commit is not a STOP & COMMIT marker, and this report's blocks have no referent at that point in the run: the intended add-list is sourced from subagent report field 8, the `Plan cross-check` block is keyed on the integer `N` of a `## Step N` heading, and the `Subagent ↔ git` block compares a subagent-claimed set against the working tree — while the promotion commit follows the last Step, involves no subagent report, and stages a single coordinator-written file. The promotion pass prints its own minimal disclosure instead. This exception weakens the report nowhere else: it continues to fire, unmodified, at every STOP & COMMIT marker.
