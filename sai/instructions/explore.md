...
10. **Supervised sai-1 pipeline (`start-pipeline`, sai-explore only)**: This is a chat-scoped supervision contract. It does not persist state, discover repository changes, or replace the post-crystallization `review-loop` contract. Explore has no direct write tool; the explicitly user-triggered exception may dispatch only the existing `sai-1-spec-proposal-worker` binding, whose writes are limited to the selected owned change directory's `openspec/changes/{name}/proposal.md`, `specs/**`, and permitted change metadata. No delegated write outside that scope is authorized.
    ...
    **Routed spec supervision (Claude Code and opencode)**: For either supported routed harness, dispatch the existing `sai-1-spec-proposal-worker` binding with an isolated `SpecWorkerRequest` whose only substantive input is the selected change's complete emitted `Ready to Propose` block:
    ...
    The selected spec worker receives only that complete `crystallized_block`, not conversation context, inferred requirements, repository discovery, or later answers. Accept only the worker's closed lifecycle payloads: `needs_input(question, options)`, `completed(changed_files)`, `failed`, or `cancelled`. Union every returned `changed_files` entry in first-seen order for supervision reporting. For every `needs_input`, forward the question and every ordered option/value exactly through the harness-native picker. Escalate exactly once per question to the user; do not infer an answer from explore context. Continue the same worker with only the user's selected answer, never the original block plus an answer or any other context.
    ...
    **Independent artifact review**: Only after the worker has generated a non-empty `proposal.md` and at least one generated file under `specs/**`, create exactly one fresh reviewer, independent of the routed lifecycle. The reviewer receives only the complete crystallized block plus freshly read generated `proposal.md` and `specs/**` artifacts. Its input excludes conversation, worker reasoning, prior reviewer state, unrelated files, writes, lifecycle bindings, continuation, and install projection. It evaluates intent and constraint coverage, proposal/spec consistency, requirement and scenario testability, and unsupported assumptions.

    Model the reviewer as an `IndependentReviewResult` with only three closed outcomes: `review_complete(findings)`, `review_failed`, or `review_cancelled`. A `review_complete` result contains an array of structured `IndependentReviewFinding` values. Each finding contains exactly the required correction data: `identifier`, `severity`, `artifact_location`, `issue_statement`, and `recommended_correction`. Reviewer output never edits artifacts directly. Accepted edits are worker-owned edits only; discarded findings require specific reasons, and a discarded finding is not silently converted into success.

    For `review_complete`, pass findings individually to the machine-feedback adapter added in Step 2. Route every resulting `needs_input` payload through the same-worker continuation rule: show the exact question and ordered options, then continue the same worker with only the user's selected answer. For `review_failed` or `review_cancelled`, report that the independent pass did not complete, do not retry or repair, and continue to the ordinary user feedback gate at iteration 0.

    **Supervised completion**: Keep `active_change` set through the ordinary `proposal.md`, `specs/**` feedback gate. After the user proceeds, add the change to `completed_changes`, clear `active_change`, suppress standalone sai-1 navigation only for this supervised run, and do not run sai-2. Emit exactly one matching terminal line:
