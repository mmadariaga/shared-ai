...
Single source of the artifact review finding contract. Every artifact review surface — the manual `sai-explore` post-crystallization review loop, the supervised pipeline's in-session review rounds (Review Engine-driven), and the automatic worker-owned planning-artifact review loop on the spec and design workers only when the invocation does not carry `--supervised` — SHALL draw its finding format from this contract by reference. User-requested worker-owned passes remain bound by this contract when the invocation carries `--supervised`. Neither `sai/commands/explore/instructions.md` nor any capability spec other than `review-finding-format` SHALL redefine the severity criteria, the finding shape, the identifier scheme, or the summary-line format inline; consuming instructions and capability specs SHALL cite this contract by reference. These semantics SHALL NOT be restated in explore or reviewer instructions.

## Scope

This contract governs **artifact review** only: read-only reviews of OpenSpec change artifacts (`proposal.md` and `specs/**` for sai-1; `design.md`, `tasks.md`, and `interfaces.md` for sai-2), whether run manually through `sai-explore`'s post-crystallization review loop or by the supervised pipeline's in-session review rounds. It does NOT govern the audit commands `sai-5-review`, `sai-6-security`, `sai-7-performance`, or `sai-8-accessibility`: those retain their own severity vocabularies and identifier schemes (e.g. `review.md`'s `C` / `H` / `M` / `L` / `Q` sections and `mMUT-N` identifiers per ADR 0013) and never emit this contract's `Summary:` tally. See the `Finding across surfaces` flagged ambiguity in the project-root `GLOSSARY.md`.
...
A finding whose severity is missing or outside the closed set SHALL NOT be assigned a derived identifier. In the supervised pipeline's in-session review rounds, every finding is formed by the Review Engine under this contract and continued to the phase worker; no reviewer output remains for pipeline-side severity validation.

## Summary tally

Every artifact review SHALL close with a `Summary:` line tallying its severity counts.

- **Base form (manual review loop and supervised rounds):** `Summary: High=<count> Medium=<count> Low=<count>` — the counts match the review's findings; no other counters. The supervised pipeline's in-session review rounds (Review Engine-driven) close with this same base-form tally.
