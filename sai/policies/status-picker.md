...
Both prompts below are closed-choice: on Claude Code and opencode, present the choices through the native option-picker; on a surface without a native picker, fall back to the plain-text prompts as written (per the "Closed-choice prompts" rule in `remember.md`). Per the concise-format rule in `@sai/policies/question-context.md`, state the full decision context in ordinary chat immediately before the picker and keep the picker question to the short summary below. The selection semantics are identical either way. Present the tool's `options` in the reported order, using each option's `label`, and never add, drop, or reorder one.
