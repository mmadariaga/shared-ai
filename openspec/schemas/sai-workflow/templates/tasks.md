## Step 1: <!-- Title -->

**Files Affected**: <!-- comma-separated file paths -->

**What Will Be Done**: <!-- prose description of the work in this step -->

**Testing Strategy**: <!-- how correctness will be verified for this step -->

## Step 2: <!-- Title -->

**Files Affected**: <!-- comma-separated file paths -->

**What Will Be Done**: <!-- prose description of the work in this step -->

**Testing Strategy**: <!-- how correctness will be verified for this step -->

## Required Documentation

### Local files

<!-- one path per line; use line ranges (e.g., path/to/file.md:10-50) when only a portion applies -->
<!-- if nothing to list, write: None -->

### External URLs

<!-- one URL per line -->
<!-- if nothing to list, write: None -->

## Required Documentation

**Local files**: <!-- List every local file consulted during design-phase codebase research -->

**External URLs**: <!-- List every external URL consulted during design-phase codebase research -->

## Implementation Context

**Stack**: <!-- primary language/framework + key versions relevant to this change -->

**Conventions**:
<!-- 2-5 project-specific, non-obvious bullets derived from observed code -->
<!-- Not acceptable: "follow SOLID principles", "write clean code", or any generic best-practice -->

**Avoid**:
<!-- anti-patterns the implementation agent might default to given the declared stack -->
