...
Give a completed `/sai-4-apply` run a durable, diffable record of how each Step was actually executed. The coordinator persists every dispatch's field-9 telemetry as rows in an `## Appendix: Execution Telemetry` section of the change's `implementation.md`, keeping the dispatch axis and the phase axis distinguishable, so a reader can later tell how a Step was routed and how many attempts each phase took without re-running anything.
...
The coordinator SHALL append a Step's telemetry rows in the same workflow loop slot that already holds the deviations appendix and the Automated checkbox marking — after the coordinator's own verification passes and **before** the coordinator commits. No human-verification confirmation SHALL condition that slot. This places the telemetry in the same commit as the work it describes.
