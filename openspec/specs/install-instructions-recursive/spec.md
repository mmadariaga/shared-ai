...
`bin/install-flow.js` SHALL export a `listMdFilesRecursive(dir)` function that returns all `.md` files under a directory tree, and `installClaude`, `installCopilot`, and `installOpencode` MUST use it to copy `sai/instructions/` so that any nested files under `sai/instructions/` land at the same relative path under the target `sai/instructions/` directory.
