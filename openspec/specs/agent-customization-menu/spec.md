...
- **WHEN** the fake settings selector presents the model or effort options for an agent
...
#### Scenario: Cancel during per-agent configuration

- **WHEN** the user presses `q` or Ctrl-C at a per-agent model or effort selection
- **THEN** customization SHALL be cancelled — the current agent and every remaining agent SHALL NOT be configured — and the flow SHALL complete normally
