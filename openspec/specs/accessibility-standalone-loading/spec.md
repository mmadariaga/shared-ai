# accessibility-standalone-loading Specification
...
The standalone run SHALL boot from the nucleus only, worker contract plus steps/common.md, with resolve-accessibility-scope carrying follow none. Each of the four step files under sai/commands/accessibility/steps/ SHALL load only when next.follow names it, and the initial dispatch SHALL bear no Active step line with the first pointer targeting map-ui-framework.
...
- **WHEN** the final step `close-accessibility-outcome` is marked completed
