...
When `/sai-backfill` is invoked with an empty `$ARGUMENTS`, the wrapper MUST forward execution to `sai/commands/backfill/instructions.md` and MUST NOT introduce any additional resolution mechanism (no inline list, no re-prompt, no `openspec list` call). The empty-`$ARGUMENTS` case is handled by the STOP Conditions section at `sai/commands/backfill/instructions.md:7-10`; the derived-name case is handled by Phase 4 ("Change Name Confirmation"). The wrapper is a pass-through to both.
...
#### Scenario: Empty arguments falls through to STOP Conditions
    ...
    - **THEN** the agent MUST print exactly `Change name required. Run: /sai-backfill <name>` and stop, as specified by the STOP Conditions section at `sai/commands/backfill/instructions.md:10`
...
#### Scenario: Non-empty arguments proceed unchanged
    ...
    - **THEN** the wrapper MUST use `some-kebab-name` as the change name and proceed to Phase 4 of `sai/commands/backfill/instructions.md` without any picker interaction
