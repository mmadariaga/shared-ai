...
- **WHEN** a coordinator considers sending the user backward because `openspec/changes/{change-name}/.openspec.yaml` lacks `approval.specs.approved_at`, and `sai/commands/design/instructions.md` declares that `/sai-2-design` writes `approval.specs.approved_at`
- **THEN** it SHALL read the file, key, and `sai/commands/design/instructions.md` in the current invocation and confirm that `/sai-2-design` is the destination command that writes `approval.specs.approved_at`
