...
After reading the diff, the command SHALL always ask the user exactly the following two questions, in this order, regardless of diff content:

1. "What problem does this solve?"
2. "What are the known limitations or technical debt left behind?"

These questions are not skipped, rephrased, or merged even if the diff appears self-explanatory.

#### Scenario: Fixed questions asked for a trivial diff
- **WHEN** the diff is a one-line rename with obvious intent
- **THEN** both fixed questions are still asked before any output is written
