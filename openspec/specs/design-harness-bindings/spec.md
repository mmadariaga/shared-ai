...
Activation SHALL include structural tests for Claude Code and opencode coordinator tool denial, design-worker artifact I/O, worker delegation, lifecycle metadata, same-worker continuation, fast-track notice behavior, and Continue-now dispatch. The envelope-forwarding test SHALL verify that the complete change request and flags travel through `arguments_value` without wrapper-label extraction or a wrapper-echo field. Claude Code and opencode SHALL each be smoke-tested for worker dispatch, source research, input relay, continuation, changed-file reporting, reconstruction, and implementation handoff. A failed required probe SHALL block activation rather than weaken delegation or persistence rules.
