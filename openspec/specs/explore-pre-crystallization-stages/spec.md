...
`sai-explore` SHALL implement a phase-A/phase-B panel lifecycle: phase A renders the stage TODO during exploration, phase B renders the idea progress list from its first render. The lifecycle rules SHALL be single-sourced in `sai/commands/explore/steps/common.md` (phase A) and `sai/commands/explore/steps/idea-list.md` (phase B). Each stage-TODO entry SHALL carry a stage-ownership marker distinct from the idea-list marker (`sai-idea-list:`) in the machine-readable panel entry field pinned by the per-harness idea-list render binding (opencode `priority`, Claude Code `description`), with the marker value `sai-explore-stage:<stage-id>`. The chat-start clear SHALL remove entries bearing either the stage-TODO marker or the idea-list marker. Each binding SHALL pin only its harness's label field and marker field for both phases and SHALL point to the two explore step files for the lifecycle rules instead of restating them. The Claude Code and opencode bindings SHALL mirror each other.
...
- **THEN** the chat-start clear removes exactly the entries bearing the `sai-explore-stage:` marker or the `sai-idea-list:` marker in the field the binding pins, leaving other surfaces' entries in place
...
#### Scenario: Idea stages 2-3 route to their own step files

- **WHEN** `explore-idea@1` is at `review-edge-cases` or `implementation-details`
- **THEN** `next.follow` names `steps/review-edge-cases.md` or `steps/implementation-details.md` respectively and `next.hint` starts with `load and follow `
...
`sai/commands/explore/steps/common.md` SHALL own the pre-crystallization TODO, next-step advancement, the shared list-stage rules, and persistence mechanics, while the `Review edge cases` and `Implementation details` stage rules SHALL live in `steps/review-edge-cases.md` and `steps/implementation-details.md`, while post-crystallization next-slice close SHALL live only in `steps/slice.md` with recordedList-before-selector order preserved.
