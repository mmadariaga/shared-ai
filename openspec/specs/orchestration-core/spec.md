...
For an eligible in-scope or owner-in-run planning diagnosis, the shared runner SHALL re-dispatch the same live worker (in-scope) or the named owner worker (owner-in-run) with the existing bounded-recovery acknowledgement and the coordinator-authored diagnosis. The coordinator SHALL never write or repair the worker-owned proposal, spec, design, task, interface, glossary, or overview artifacts. An out-of-scope or unresolved cause SHALL spend zero recovery slots and SHALL stop through the phase-owned hand-back. The worker remains responsible for applying the correction and verifying the artifacts before returning `completed`.
...
- **AND** if out-of-scope, SHALL name that prior-phase artifact and concrete point as out of scope, spend zero recovery slots, and not edit the spec artifact
...
- **WHEN** a worker returns `unrecoverable: true` and coordinator inspection appears to identify a safe in-scope or owner-in-run correction
...
For an opted-in phase-adapter segment, the recovery budget SHALL be a ledger of exactly three available diagnosis slots, not a reusable retry counter. Each eligible in-scope or owner-in-run diagnosis SHALL have one coordinator-authored normalized `diagnosis_key` derived from the concrete cause evidence and correction boundary. A new key SHALL spend exactly one slot and permit at most one same-worker or owner-worker recovery attempt; a key already present in the ledger SHALL be rejected as a duplicate before dispatch and before spending a slot, and recovery SHALL stop even if an unused slot remains. The worker-authored `failure_class` SHALL be a diagnostic prior, not the eligibility gate: with `unrecoverable: false`, a coordinator-proven clear safe in-scope or owner-in-run cause MAY make even `blocking-contradiction` or `unclassified-worker-fault` eligible, while an out-of-scope or unresolved Cause Locus SHALL spend zero regardless of class. A changed worker failure class, routing diagnosis, or summary SHALL not make the same concrete cause a new key. Malformed and continuation/transport-loss cases SHALL use their existing zero-attempt hand-back or stop behavior.
...
- **WHEN** a failed worker result carries `blocking-contradiction` or `unclassified-worker-fault` with `unrecoverable: false` and coordinator evidence proves a clear safe in-scope or owner-in-run correction
...
- **WHEN** a worker result carries an otherwise eligible class but coordinator evidence proves the cause is outside the worker's authorized scope and no in-run owner holds the correction boundary
...
- **WHEN** an eligible in-scope or owner-in-run closure has a `diagnosis_key` not present in the active segment ledger
- **THEN** the coordinator SHALL record that key and spend exactly one of the three slots
- **AND** it SHALL permit at most one same-worker or owner-worker recovery continuation for that key
...
- **AND** the later key MAY spend a remaining slot only after the coordinator proves an in-scope or owner-in-run safe correction

#### Scenario: Out-of-scope diagnosis has no ledger entry
- **WHEN** concrete evidence proves that a non-clean cause is outside the active worker's authorized scope and no in-run owner holds the boundary
...
- **THEN** it SHALL invoke the validator with `--kind notice`, `--kind progress`, or `--kind conflict_detected` respectively, except that a progress payload under a `step_machine` adapter is validated with kind `progress` inside the `sai-state` progress emit
