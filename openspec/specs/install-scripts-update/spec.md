...
`INSTALL.claude.md` SHALL gain a `cp` line that installs the universal skill into the Claude Code skills directory, following the same pattern as the caveman skill copy step.
...
It SHALL be placed immediately after the caveman copy step (currently line 46) and before the budget-explorer copy step.
...
The bash installation section of `README.md` (around line 247–254) SHALL gain a `cp` line for the token-efficient-languages skill, following the same pattern as the caveman skill:

```
cp skills/universal/token-efficient-languages/SKILL.md ~/.config/opencode/skills/token-efficient-languages/SKILL.md
```

It SHALL be placed immediately after the caveman copy step and before the budget-explorer copy step.
...
The PowerShell installation section of `README.md` (around line 293–300) SHALL gain a `Copy-Item` line for the token-efficient-languages skill, following the same pattern as the caveman skill:

```
Copy-Item skills\universal\token-efficient-languages\SKILL.md "$configDir\skills\token-efficient-languages\SKILL.md"
```

It SHALL be placed immediately after the caveman `Copy-Item` step and before the budget-explorer `Copy-Item` step.
...
- **THEN** no separate `mkdir` for the skill's parent directory is required (the parent exists from caveman/budget steps above)
