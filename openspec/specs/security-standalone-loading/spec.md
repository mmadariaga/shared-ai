# security-standalone-loading Specification

## Purpose
TBD - created by archiving change add-security-step-machine. Update Purpose after archive.
## Requirements
### Requirement: Closed standalone stage table for security

The machine SHALL expose exactly the five progress-plan ids in order as its closed states: `resolve-security-scope`, `discover-module-map`, `resolve-sast-analysis`, `resolve-sca`, `close-security-outcome`, plus terminal `done`. The STEPS list, STAGE_FILES mapping, DONE_STAGE marker, and initialState with empty done set SHALL match the coordinator plan with no extra states.
...
The standalone run SHALL boot from the nucleus only, worker contract plus steps/common.md, with resolve-security-scope carrying follow none. Each of the four step files under sai/commands/security/steps/ SHALL load only when next.follow names it, and the initial dispatch SHALL bear no Active step line with the first pointer targeting discover-module-map.
...
- **WHEN** the final step `close-security-outcome` is marked completed
...
The machine SHALL govern the security adapter for every activation — direct (`/sai-6-security`) and chained (as position 1 of `/sai-review`). No shared sessions or state exist between activations. Pointer sequences remain byte-identical between direct and chained paths. Feedback and recovery continuations SHALL carry no pointer and never consult the machine; parking and pointer behavior follow `@sai/policies/stage-machine.md` § Step machines.

#### Scenario: Direct activation consults the machine

- **WHEN** `/sai-6-security` is invoked directly
- **THEN** the machine arbitrates step routing via the coordinator's declared `step_machine: security-standalone@1` and delivers the two-line continuation

#### Scenario: Chained activation consults the same machine

- **WHEN** the security adapter runs as position 1 of a chained `/sai-review` invocation
