...
#### Scenario: Diagnosis-entry worker result resolves the item before diagnosis

- **WHEN** the phase worker returns `failed`, coordinator-disproved `completed`, STOP-bearing `completed`, or `cancelled` while supervised review rounds are in progress
- **THEN** the active phase item resolves to `pending` before the item-10 Diagnosis Round begins
...
When the slicing assessment (`sai/commands/explore/steps/slicing-assessment.md`) identifies a slice, its slice-crystallization item and its two review items SHALL be added in the unmarked state. The slice-crystallization item SHALL be marked when that slice's `Ready to Propose` block is emitted; the two review items SHALL remain unmarked, awaiting review-pass evidence.
...
### Requirement: completed slice progress persists during Direct Build continuation
...
- **WHEN** a clean Direct Build completion is followed by pending slices
- **THEN** completed entries are not cleared, rewound, or reclassified

### Requirement: Idea-list detail lives in idea-list step
`sai/commands/explore/steps/idea-list.md` SHALL hold the full idea-list Phase A and Phase B detail with stage-ownership and route-ownership prefixes unmixed, while `sai/commands/explore/instructions.md` SHALL keep only a step-map pointer to it, reached via next.follow.
