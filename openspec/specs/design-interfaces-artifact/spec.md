...
`interfaces.md` SHALL admit **no** leading non-`Step N` top-level section: the `## Target State` section and its `### Architecture Snapshot` and `### File Manifest` subsections, defined by the `design-target-state` and `change-overview-artifact` capabilities, SHALL be emitted in `design.md` and `change-overview.md` and SHALL NOT be emitted in `interfaces.md`. Every top-level section of `interfaces.md` SHALL be a `## Step N` section, with exactly one admitted exception: the `None — no step contracts` sentinel defined by the `interfaces.md is never empty` requirement of this capability, which is the sole content when no step admits a contract. No other non-`Step N` section SHALL appear anywhere in the file. The per-step keying rule governs every `## Step N` section of `interfaces.md`.
...
#### Scenario: no non-step top-level section is admitted
- **WHEN** `sai-2-design` generates `interfaces.md`
- **THEN** every top-level section of `interfaces.md` is a `## Step N` section
- **AND** no `## Target State` section and no `### Architecture Snapshot` or `### File Manifest` subsection appears in the file — that content lives in `change-overview.md`
...
### Requirement: interfaces.md is never empty

`interfaces.md` SHALL always be written and SHALL always be non-empty, even when no step introduces a new/modified public interface and no step carries a testable assertion. When every `## Step N` section would be omitted under the per-step omission rule, `interfaces.md` SHALL instead carry the exact sentinel `None — no step contracts` followed by a one-line reason naming why no step admits a contract. The sentinel is the sole content of the file in that case, keeps the design worker's exists-and-non-empty verification satisfied, and keeps the `change-overview` artifact's `requires: [interfaces]` dependency satisfiable for every designed change.

#### Scenario: sentinel emitted when no step admits a contract
- **WHEN** every step of a change introduces neither a new/modified public interface nor a testable assertion
- **THEN** `interfaces.md` exists, is non-empty, and contains exactly the sentinel `None — no step contracts` plus a one-line reason
- **AND** the design worker's non-empty verification passes, and the overview's `requires: [interfaces]` dependency is satisfied

#### Scenario: no empty interfaces.md under any design
- **WHEN** `sai-2-design` completes for any change
- **THEN** `interfaces.md` exists and is non-empty — either `## Step N` sections or the `None — no step contracts` sentinel
