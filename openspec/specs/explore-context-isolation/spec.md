...
`sai-explore` SHALL NOT create a change proposal or run `/sai-1-spec` inline during an explore session. When an idea crystallizes and is ready to formalize, `sai-explore` SHALL offer a structured summary block that the user can copy and paste into `/sai-1-spec` in a new session.

The summary block SHALL contain:
...
#### Scenario: idea crystallizes during exploration

- **WHEN** the user's idea becomes clear enough to propose during a `sai-explore` session
...
- **THEN** `sai-explore` explains that creating a proposal opens a new context and offers the paste-ready summary block instead, so the user can start that new session cleanly
