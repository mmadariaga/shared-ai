...
The cold-build branch in `sai/commands/implement/steps/decision-record-index.md` MUST read the ADR template from `sai/commands/implement/adr-index.template.md` and the DDR template from `sai/commands/implement/ddr-index.template.md` by exact path, without inlining their boilerplate.

#### Scenario: Build a missing decision-record index

- **WHEN** the decision-record pass creates an ADR or DDR index because that family index is absent
