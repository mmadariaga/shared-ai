...
- **WHEN** the review presents a non-empty proposed list
- **THEN** the emission is one message running the formal list, then the lightweight probes, then the single agreement question

#### Scenario: Fewer than 2 material scenarios are declared explicitly

- **WHEN** fewer than 2 material probe scenarios exist
- **THEN** the review shows what the probes produced and states explicitly that there are no more material scenarios, and does not pad the emission with non-material probes
...
When the user semantically agrees that the proposed list is correct, `sai-explore` SHALL finalize that list for crystallization and advance the stage progression to the `Implementation details` stage within the same turn. A disagreement SHALL keep the review open, incorporate the requested changes, and ask the agreement question again. Explanations of side effects SHALL be owed only when user feedback drags changes into `E` items other than the one the feedback addressed (ripples); feedback aimed at a shown probe SHALL bind to the `E` item that probe illustrates. An ambiguous response SHALL be treated as discussion rather than agreement and SHALL cause the question to be clarified and re-asked.
...
#### Scenario: Ripples are explained and probe feedback binds to its illustrated item

- **WHEN** user feedback drags changes into `E` items other than the one the feedback addressed, or is aimed at a shown probe
- **THEN** `sai-explore` explains the dragged ripple effects, and feedback aimed at a shown probe binds to the `E` item that probe illustrates
...
When the user advances to the `Review edge cases` stage and no behavior bounds the proposed change, `sai-explore` SHALL emit exactly `- None` with no probes run, record the empty list as agreed, and advance the stage progression to the `Implementation details` stage without running the agreement question — no iteration. This deterministic empty-set rule mirrors the implementation-details stage's rule (`explore-implementation-details`) and never fires any gate or questionnaire.
...
- **THEN** `sai-explore` emits `- None` with no probes run, records the empty agreed list, and the progression advances to `Implementation details` without a confirmation question
...
- **THEN** the empty list is recorded as agreed by the empty-set rule and the pending crystallization request proceeds through the slicing assessment and both language gates
