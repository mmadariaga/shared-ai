...
- **THEN** it does NOT dispatch a test-writer or an implementation worker; it routes the Step to the RED worker under the green-exception, which authors the tests and leaves them green

#### Scenario: RED worker does not verify a valid RED

- **WHEN** the RED worker's report indicates RED result `passes` or `wrong-failure`
- **THEN** the coordinator does NOT dispatch the GREEN worker, and surfaces the invalid RED to the user per the RED handling rules
