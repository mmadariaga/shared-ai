...
When `openspec/changes/{change-name}/implementation.md` exists before `/sai-3-implement` runs, the agent SHALL read the file, identify every `#### Step N:` section where all checkboxes are `[x]`, and emit only `*(already applied)*` in place of that step's full content in the regenerated output. Steps with at least one unchecked `- [ ]` checkbox SHALL be emitted in full.
...
- **THEN** the regenerated `implementation.md` contains only step headings followed by `*(already applied)*`, with no code blocks or checklists
...
- **THEN** that step is NOT collapsed — it appears in full in the regenerated output
