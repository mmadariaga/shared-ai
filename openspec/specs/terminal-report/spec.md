...
The system SHALL report findings as screen-only non-blocking warnings with no artifact write, no selector, no auto-fix, and no recovery continuation, printing each fail and unverifiable check with its reason plus one recommendation line, preceding the unchanged completion literal.
#### Scenario: Findings print before unchanged literal
- **WHEN** the terminal review completes with fail or unverifiable verdicts
- **THEN** each finding prints with its reason and one recommendation line before the byte-identical completion literal with no file write and no selector
...
The system SHALL print under fast-track in the fixed order of findings, then the deferred Human Verification list, then the unchanged completion literal.
#### Scenario: Fast-track findings-HV-literal order
- **WHEN** fast-track is active and the terminal review has completed
- **THEN** the output order is findings first, deferred Human Verification list second, and completion literal last
