...
3. The fix has no meaning in the other harnesses, or would be actively wrong noise if mirrored (e.g. adding a harness-only wrapper substitution workaround where `$ARGUMENTS` is substituted into the body file directly in the other harnesses).
