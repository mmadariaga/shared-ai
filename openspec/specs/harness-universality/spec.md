...
The mirror discipline SHALL apply to this cross-cutting envelope migration because the same behavior is meaningful in Claude Code and opencode. Removing the retired wrapper-echo field SHALL be implemented in both supported harness projections and their shared neutral contracts; it SHALL not be classified as a one-harness adapter exception.
