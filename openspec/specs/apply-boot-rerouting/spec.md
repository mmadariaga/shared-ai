...
Because `apply` is a routed card, the routed apply coordinator SHALL dispatch RED and GREEN workers through their bindings, per the neutral command-runner protocol. The boot adapters SHALL forward `command_name` for card selection and `arguments_value` byte-for-byte to the apply coordinator.
...
- **THEN** it forwards `arguments_value` byte-for-byte and leaves worker dispatch and continuation to the worker binding the coordinator loads
