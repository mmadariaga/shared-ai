...
- **WHEN** `sai/instructions/design.md` presents an unresolved Open Question to the user
...
`sai/instructions/apply.md` SHALL reference the question-context policy at its two fixed decision surfaces: the Human Verification gate presentation and the GREEN-conflict halt presentation. Both prompts SHALL comply with the anatomy while preserving the gate's existing mechanics (checkbox-keyed gate, `Human Verification` wording, and the halt semantics).
