...
- **AND** it runs another round only when a completed round contains a `High` finding and the three-round bound permits it
...
- **THEN** it runs at most three review rounds for the design phase, counted under the design phase's own counter per the `supervised-review-rounds` capability
...
#### Scenario: user design gate is deferred while another round remains
- **WHEN** a completed design round contains a `High` finding and another round remains within the three-round bound
...
#### Scenario: user design gate opens after the rounds terminate
- **WHEN** the design review rounds converge or exhaust the three-round cap
