...
```
 ...
 sai/commands/{explore,pr,retire-docs,status,worktree}/          ← utility cards: body.md is the command body
 ...
 bin/sai-state.js                 ← stage machine CLI (spawn / emit / close)
...
| Directory | Purpose |
|-----------|---------|
| `sai/commands/{name}/instructions.md` | Command-local phase content (TASK block), fetched by the card that owns it. Present in 12 of the 19 card folders; `apply`, `design`, `meta-build`, `meta-review`, `retire-docs`, `spec`, and `status` carry none — their content lives in `coordinator.md`/`worker.md`/`steps/` instead. |
...
| `sai/commands/{explore,pr,retire-docs,status,worktree}/` | Utility cards: `body.md` is the complete command body. `explore`, `pr`, and `worktree` additionally carry an `instructions.md` with their phase content; `retire-docs` and `status` are body-only. |
