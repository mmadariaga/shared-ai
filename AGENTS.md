...
**Active experiment — step-gated instruction delivery:** `/sai-1-spec` and `/sai-3-implement` each split the worker's instruction mass into one file per progress-plan step under `sai/commands/{spec,implement}/steps/` (`common.md` plus per-id step files; `prereqs-and-change` / `prereqs-resolution` has no file of its own). The worker card loads `steps/common.md` at dispatch alongside its fixed protocols; each coordinator declares a static `step_pointer_map`, and each progress-event continuation carries one deterministic `Active step:` pointer line naming the next step file. Feedback and recovery continuations carry no pointer line, replacement reconstruction adds `active_step_id`, and adapters without a declared map keep today's exact continuation behavior, leaving the other eight phases observationally identical. `/sai-build`'s chained implement segment rebinds the implement adapter's declared `step_pointer_map` through per-segment adapter-field rebinding and therefore inherits pointer delivery, with no opt-out special case.
