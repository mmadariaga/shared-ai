...
| `sai/commands/spec/invocation.md`, `sai/commands/implement/invocation.md`, and `sai/commands/apply/invocation.md` | Caller-neutral invocation bodies retained by the routed paths; `review`, `security`, and `accessibility` keep equivalent invocation bodies. Design instruction delivery is step-gated under `sai/commands/design/steps/`; the build composition uses `sai/commands/meta-build/command-bootstrap.md` and `coordinator.md`. |
