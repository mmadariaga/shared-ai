...
```
 ...
 sai/commands/implement/ddr-index.template.md ← canonical project-agnostic DDR index template, owned by the implement command
  ...
  sai/orchestration/workers/bindings/ ← neutral installed routed worker bindings (ten phases plus the two apply Step-execution workers)
 ...
 agents/claude/                   ← Claude Code managed agents (twelve routed Managed Workers + three Generic Agents)
...
| `sai/orchestration/workers/bindings/` | Neutral installed routed worker bindings for the ten phases (`spec/design/implementation/review/security/performance/accessibility/commit/archive/backfill-worker.md`) plus the two apply Step-execution worker bindings (`red-worker.md`/`green-worker.md`) projected for both harnesses; non-worker panel and idea-list runtime glue is owned by the adapter seam. |
