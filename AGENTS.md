...
**Active experiment — step-gated instruction delivery:** `/sai-1-spec` splits the worker's instruction mass into one file per progress-plan step under `sai/commands/spec/steps/` (`common.md` plus `research.md`, `proposal.md`, `specs.md`, `validation.md`, `review.md`). The worker card loads `steps/common.md` at dispatch alongside its fixed protocols; the spec coordinator declares a static `step_pointer_map`, and each progress-event continuation carries one deterministic `Active step:` pointer line naming the next step file (`prereqs-and-change` has no file of its own — it runs from the worker card plus common.md before the first pointer). Feedback and recovery continuations carry no pointer line, replacement reconstruction adds `active_step_id`, and adapters without a declared map keep today's exact continuation behavior, leaving the other nine phases observationally identical.
