...
```
 ...
 sai/commands/                    ← command cards — 19 folders; every folder has command-bootstrap.md (fetched by the wrapper after the boot adapter)
 sai/commands/{spec,design,implement,apply,review,security,performance,accessibility}/  ← routed phase cards: coordinator.md + command-bootstrap.md; worker.md except apply (red-worker.md / green-worker.md); invocation.md on apply and the four audits
 sai/commands/meta-build/             ← routed composition cards: command-bootstrap.md + coordinator.md (no invocation.md; no worker of its own)
 ...
 sai/commands/merge/              ← routed-shaped cards: coordinator.md + worker.md + lifecycle.md + presentation.md (no invocation.md; no openspec dependency)
 sai/commands/{explore,pr,retire-docs,status,worktree}/          ← utility cards: body.md is the command body (boot selects body.md, not a coordinator)
 sai/commands/{name}/instructions.md   ← command-local phase content (TASK block); present in 12 of the 19 card folders
 sai/commands/{name}/*.template.md     ← neighboring co-located report/plan template files beside each card
 sai/commands/{name}/steps/            ← step libraries: worker instruction files (spec/design/implement/review/security/performance/accessibility), explore main-session steps, apply coordinator routing files
 sai/commands/design/change-overview.md      ← shared overview-generation instruction, owned by the design command
 sai/commands/design/phase-contract.md       ← canonical design progress plan, write surface, and result union
 ...
 sai/commands/explore/direct-build-worker.md ← Direct Build implementer contract (budget-tier matrix role)
 ...
 sai/orchestration/workers/bindings/ ← installed routed worker bindings (eleven phases plus the two apply roles and the one direct-build implementer)
 ...
 commands/claude/                 ← Claude Code wrappers (model + effort + fetch boot.md + command-bootstrap.md)
 commands/opencode/               ← opencode wrappers (model + fetch boot.md + command-bootstrap.md)
 ...
 bin/                             ← installer, doctor, uninstall, worker-matrix, and sai-state CLI
...
| Directory | Purpose |
|-----------|---------|
| `sai/commands/{name}/instructions.md` | Command-local phase content (TASK block). Present in 12 of the 19 card folders; `apply`, `design`, `meta-build`, `meta-review`, `retire-docs`, `spec`, and `status` carry none. Not every copy is on the live fetch route: `implement/instructions.md` is a cited reference (review, adr-ddr-criteria, plan-generation); audit `instructions.md` files are fetched only from unused `invocation.md` shells. Explore, pr, worktree, archive, backfill, commit, and merge load theirs from the live card. |
...
| `sai/orchestration/workers/bindings/{claude,opencode}/worker-template.md` | The two checked-in per-harness binding templates. The **installed** bindings are materialized from them plus `worker-matrix` at projection time — eleven phase bindings (`spec/design/implementation/review/security/performance/accessibility/commit/archive/backfill/merge-worker.md`), the two apply Step-execution bindings (`red-worker.md`/`green-worker.md`), and the explore implementer binding (`direct-build-worker.md`). No named binding file is checked in; a `Fetch @sai/orchestration/workers/bindings/spec-worker.md` resolves against the installed projection. Direct-build draft and archive execution reuse the backfill and archive bindings; non-worker panel and idea-list glue is owned by the adapter seam. |
...
| `skills/claude/` | Claude Code harness skills. |
| `skills/opencode/` | opencode harness skills. |
| `skills/` | Universal skills installed globally (not project-local). Resolved via `~/.claude/skills/` or `~/.config/opencode/skills/`. Wrappers fetch only the harness `fetch` skill; budget / safe-operations / sai-commands load on cards. |
...
| `skills/universal/budget/SKILL.md` | Loads explorer + executor + subagent + token-efficient-languages together. Fetched by explore `body.md`, apply `invocation.md`, backfill `worker.md`, and `steps/common.md` for spec/design/implement/review/security/performance/accessibility. |
...
| `skills/claude/budget-explorer/SKILL.md` | Subagent dispatch rules for Claude Code — dispatch uses the matching `budget-explorer` agent file, whose `model` and `effort` frontmatter is user-owned; task classification, a 30-call maximum, and output contracts. Loaded via `@skills/budget/SKILL.md`, not from wrappers. |
...
| `commands/claude/` | Wrappers for Claude Code. YAML frontmatter (`description`, `argument-hint`, `model`, `effort`) + fetch to `sai/adapters/claude/boot.md` + fetch to `sai/commands/{name}/command-bootstrap.md`. |
...
Claude Code and opencode route `/sai-3-implement` through the shared orchestration core and their respective worker binding. Both preserve the same `implementation.md` artifact contract and MANDATORY STOP. The routed coordinator performs no technical I/O and only the worker owns routed planning writes. The coordinator owns `--fast-track` parse and banner per `@sai/policies/fast-track-flag.md` and declares `fast_track_active` as invocation-scoped session state; the worker never parses the token.
...
Self-gated Step routing uses `apply-standalone@1` and one file from `sai/commands/apply/steps/` per Step (degraded-store fallback loads every routing file). The run-start Step Projection seeds that machine from on-disk `#### Step N:` headings and checkbox state.
...
All sai-* change artifacts (`implementation.md`, `review.md`, `security.md`, `performance.md`, `accessibility.md`, `pr.md`) write to `openspec/changes/{change-name}/`. Apply may also write project-root `SAI_LEARNINGS.md` (format: `sai/policies/sai-learnings-format.md`). The legacy `plans/` directory is **not used** by the new pipeline.
...
### No-commit guard
Worker "never run a mutating git command" rules are prose the worker reads at start; the no-commit guard turns the one invariant true in every project — HEAD immobility across a dispatch — into a filesystem check. `sai/tools/no-commit-guard.js` (`snapshot`/`verify`, JSON verdicts `clean | violation | allowed | n/a`) is single-sourced in `sai/policies/no-commit-guard.md`: every coordinator card that dispatches a routed worker binding (spec, design, implement, apply, review, security, performance, accessibility, commit, archive, backfill, merge, meta-review) plus explore's three Direct Build windows gains a snapshot immediately before each dispatch/continuation (or batch start) and a verify after each result (or batch close), before acting on it. The baseline SHA lives in coordinator conversation state as `guard_base` (literal `n/a` when HEAD cannot be resolved; never persisted); on violation the coordinator captures the payload's commit evidence, runs `git reset <guard_base>` (mixed), prints the one pinned incident line from `@sai/policies/autonomy-audit-log.md` (§ Incident line), and continues the route — the remediation reset is pre-authorized by the flow and carved out of safe-operations' confirmation gate. `allow_commit` is the lax dispatch-carried flag that permits HEAD movement; its only carrier is the archive worker's pre-authorized Direct Build `--direct-build-execute` continuation. Coordinator-owned mutations (apply gates, merge launch, the archive commit gate) always run between windows; worker prose prohibitions stay unchanged.

### Explore-mode read-only enforcement
`sai-explore`'s explore-mode "No file writes" guarantee (`sai/commands/explore/instructions.md`) is **enforced**, not merely conventional, in Claude Code via `allowed-tools` (scoped to read/search/dispatch tools — `Edit`, `Write`, and bare `Bash` omitted; shell limited to `Bash(openspec:*)`, `Bash(git:*)`, and the node-scoped `prereqs.js` / `research-tools-check.js` / `sai-state.js` globs; `WebFetch`, `WebSearch`, and `mcp__codegraph__codegraph_explore` are also granted). **opencode** has no per-command tool-restriction frontmatter field, so `commands/opencode/sai-explore.md` is intentionally left unchanged and its read-only guarantee stays model-discipline-only; routing opencode's `sai-explore` to a read-only sub-agent was rejected because it breaks the main-session interactivity the command requires.
...
### RED → GREEN
Integrated in `implementation.md` (authored by `sai-3-implement`) and executed by the apply RED/GREEN workers (`sai/commands/apply/red-worker.md`, `green-worker.md`). `sai/commands/implement/instructions.md` remains a cited reference for the RED-block rule; apply does not fetch it.
...
Live `Fetch @sai/policies/change-picker.md` sites today:
...
- Fetch URLs point to `@~/.claude/sai/{commands,policies,orchestration,tools}/...` (Claude Code) or `@~/.config/opencode/sai/{commands,policies,orchestration,tools}/...` (opencode).
