...
| `sai/orchestration/workers/bindings/` | Neutral installed routed worker bindings for the eleven phases (`spec/design/implementation/review/security/performance/accessibility/commit/archive/backfill/merge-worker.md`) plus the two apply Step-execution worker bindings (`red-worker.md`/`green-worker.md`) and the explore auto-fast implementer binding (`autofast-implement-worker.md`) projected for both harnesses; Auto-fast draft and archive execution use the existing backfill and archive bindings; non-worker panel and idea-list runtime glue is owned by the adapter seam. |
