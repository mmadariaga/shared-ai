...
- `sai-explore` — skips both language gates (artifact review and crystallization), and at a subsequent Auto activation skips the overview-language ask with `overview_language` resolving to `None` (an explicit `--overview-lang` still suppresses the ask in every mode; under Auto (fast implementation) it is a documented no-op).
