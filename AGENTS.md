...
```
 ...
 agents/claude/                   ← Claude Code managed agents (fourteen routed Managed Workers + three Generic Agents)
...
| `sai/orchestration/workers/bindings/` | Neutral installed routed worker bindings for the ten phases (`spec/design/implementation/review/security/performance/accessibility/commit/archive/backfill-worker.md`) plus the two apply Step-execution worker bindings (`red-worker.md`/`green-worker.md`) and the two explore auto-fast role-worker bindings (`autofast-implement-worker.md`/`autofast-hands-worker.md`) projected for both harnesses; non-worker panel and idea-list runtime glue is owned by the adapter seam. |
...
**Active experiment — step-gated instruction delivery:** `/sai-1-spec` splits the worker's instruction mass into one file per progress-plan step under `sai/commands/spec/steps/` (`common.md` plus `research.md`, `proposal.md`, `specs.md`, `validation.md`, `review.md`). The worker card loads `steps/common.md` at dispatch alongside its fixed protocols; the spec coordinator declares a static `step_pointer_map`, and each progress-event continuation carries one deterministic `Active step:` pointer line naming the next step file (`prereqs-and-change` has no file of its own — it runs from the worker card plus common.md before the first pointer). Feedback and recovery continuations carry no pointer line, replacement reconstruction adds `active_step_id`, and adapters without a declared map keep today's exact continuation behavior, leaving the other nine phases observationally identical. `/sai-2-design` replicates the convention under `sai/commands/design/steps/` (`common.md` plus `research.md`, `design.md`, `tasks.md`, `interfaces.md`, `review.md`, `overview.md`); the design coordinator declares a static `step_pointer_map` over both plans' id superset (the base-plan activations never derive the inert `overview` pointer entry), the design worker card loads `steps/common.md` at dispatch and executes only the coordinator-named active step, `steps/review.md` and `steps/overview.md` are thin reinforcement files (review references `@sai/policies/artifact-feedback-gate.md`; overview references the worker card's Overview generation section, which stays verbatim because the test suite pins those mechanics in the card), reconstruction gains `active_step_id`, and `invocation.md` and `instructions.md` remain untouched.
