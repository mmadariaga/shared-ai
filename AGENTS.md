...
```
 ...
 sai/commands/{explore,pr,status,worktree}/          ← utility cards: body.md only
 ...
 sai/commands/implement/ddr-index.template.md ← canonical project-agnostic DDR index template, owned by the implement command
  ...
  sai/orchestration/workers/bindings/ ← neutral installed routed worker bindings (ten phases plus the two apply and the two auto-fast role workers)
...
| `sai/commands/merge/` | Routed-shaped merge cards: `coordinator.md` + `worker.md` (no `invocation.md`; no openspec dependency — the documented exemption). The worker owns the read-only analysis (pre-merge checks, branch selection, conflict analysis, resolution proposals, verification loop, ADR/DDR collision scanning) and returns gate questions as `needs_input`; the coordinator owns every mutation — the merge launch, resolution writes, ADR/DDR renames, reference updates, staging, and the authorized commit. |
| `sai/commands/{explore,pr,status,worktree}/` | Utility cards: `body.md` only — the complete command body for utility commands. |
...
| `sai/orchestration/workers/bindings/` | Neutral installed routed worker bindings for the eleven phases (`spec/design/implementation/review/security/performance/accessibility/commit/archive/backfill/merge-worker.md`) plus the two apply Step-execution worker bindings (`red-worker.md`/`green-worker.md`) and the two explore auto-fast role-worker bindings (`autofast-implement-worker.md`/`autofast-hands-worker.md`) projected for both harnesses; non-worker panel and idea-list runtime glue is owned by the adapter seam. |
...
| `sai/compat/` | Caller-neutral design/implementation invocation cores and shared compatibility assets. The ADR index template is not owned here. |
| `sai/commands/design/invocation.md`, `sai/commands/implement/invocation.md`, and `sai/commands/apply/invocation.md` | Caller-neutral invocation bodies shared by the routed paths; `review`, `security`, and `accessibility` keep equivalent invocation bodies. The routed spec phase uses `sai/policies/spec-phase-contract.md` directly. The build composition uses `sai/commands/build/command-bootstrap.md` and `coordinator.md`. |
...
| `agents/claude/` | Claude Code managed agents — fifteen routed Managed Workers (`sai-1-spec-proposal-worker`, `sai-2-design-worker`, `sai-3-implementation-worker`, `sai-4-red-worker`, `sai-4-green-worker`, `sai-5-review-worker`, `sai-6-security-worker`, `sai-7-performance-worker`, `sai-8-accessibility-worker`, `sai-commit-worker`, `sai-archive-worker`, `sai-backfill-worker`, `sai-merge-worker`, `sai-autofast-implement-worker`, `sai-autofast-hands-worker`) plus the three Generic Agents (`budget-explorer`, `budget-executor`, `budget-subagent`). |
...
Claude Code and opencode route `/sai-merge` through the routed-shaped merge card set (`sai/commands/merge/coordinator.md` + `worker.md`) and dispatch the `sai-merge-worker` managed worker through the `merge-worker.md` binding. The adapter declares the same minimal lifecycle shape as commit and archive: no `progress_plan`, no `recovery_policy`, no progress events, closed adapter fields; `original_envelope` is exactly `arguments_value`. The coordinator keeps the openspec prerequisite exemption (like `sai-commit`), parses `--fast-track` itself (banner + cleaned remainder), and declares `fast_track_active` to the worker as session state alongside the envelope — never an envelope key. The worker owns the read-only analysis (pre-merge checks, branch selection, conflict analysis with ours/theirs/base, resolution proposals — semantic merge for `specs/**`, guided fusion for code — verification loop up to 3 rounds, and ADR/DDR collision scanning with suffix assignment by ascending commit date and repo-wide reference-update proposals) and returns gate questions as `needs_input` results that the coordinator presents through the native picker and forwards verbatim. The coordinator owns every mutation: the merge launch, resolution file writes, ADR/DDR renames (`git mv`), reference updates, staging, and the authorized merge commit (HEREDOC form). Terminal navigation prints the worker-authored summary verbatim and closes with exactly `Merge done.` only when the commit executed; refusal documents the exact repo state (E9). Both harnesses preserve the same payloads, stop texts, gate question wordings, scope-gate semantics, verification budget, ADR/DDR suffix ordering, and commit surface.

### Auto-fast selector option (crystallization-close)
Claude Code and opencode offer a third **Auto (fast implementation)** option on sai-explore's crystallization-close selector (item 10 of `sai/commands/explore/instructions.md`) beside the existing Auto/Manual options. Selecting it consents delegated writes AND pre-authorizes exactly one local commit; it dispatches nothing unless selected and every other command surface is unaffected. The run is a seven-step unattended flow: block-driven direct implementation by `sai-autofast-implement-worker` (envelope = `--autofast` marker + the complete Ready to Propose block — it receives ONLY the block), a functional review/fix loop with the existing three-round budget semantics (cap exhaustion non-failure), retroactive artifact reconstruction via the EXISTING sai-backfill worker with grounded auto-answer-or-escalation (diff source = staged changes over a base SHA captured at implementer dispatch; Q1 from What/Why; Q2 from fix-loop findings), spec review plus schema validation of draft content read-only against `openspec/schemas/sai-workflow/schema.yaml`, the ADR/DDR criteria pass, archive mutations under fast-track gate semantics via the EXISTING sai-archive worker pre-flight, and closed-order validated mutations by `sai-autofast-hands-worker` (exact-path writes → sync → archive move → owned-path staging → commit-rules message authoring → HEREDOC local commit; never push). Both workers are budget-tier managed workers wired end-to-end like the apply RED/GREEN roles: matrix entries in `sai/install-manifest.json` (`autofast-implement` / `autofast-hands` phases pinning their explore-owned contracts at `sai/commands/explore/autofast-{implement,hands}-worker.md`), materialized bindings for both harnesses, and managed agent files. The internal name "auto-fast" remains provisional.
...
### Prerequisite check
All openspec-dependent sai-* commands (`sai-explore`, `sai-1-spec`, `sai-2-design`, `sai-3-implement`, `sai-4-apply`, `sai-build`, `sai-archive`, `sai-5-review`, `sai-6-security`, `sai-7-performance`, `sai-8-accessibility`, `sai-pr`) perform three checks by fetching `@sai/policies/prereqs.md` (resolved per harness: Claude Code via `~/.claude/sai/`, opencode via `~/.config/opencode/sai/`): (1) `openspec` binary in PATH, (2) `openspec/` directory exists, (3) `openspec/config.yaml` declares `schema: sai-workflow`. `/sai-build` performs resolution and prerequisite checks once for the composition; its apply segment does not repeat them. `sai-commit`, `sai-merge`, and `/sai-worktree` are the only exceptions — they operate on git state only and work in projects without openspec. Routed-shaped `/sai-commit` and `/sai-merge` keep that exemption in their coordinator cards: they never fetch `@sai/policies/prereqs.md` and require no openspec artifact or binary. `/sai-worktree` runs one additional best-effort, non-fatal step after creating a worktree: it announces and runs `codegraph init <worktree-path>` there, printing a single one-line result notice whether it succeeds, the `codegraph` binary is absent, or it fails — while still requiring no OpenSpec prerequisites.
