# DDR 0151c: External Explore findings block is the only `review` progress evidence

<!-- adr-index: amends 0133b; refs adr:0158c -->
...
- Amends DDR 0133b: the evidence source is an external Explore findings block, not a worker-owned pass.
- Gate note (ADR 0160b) advertises `sai-explore` `review-loop` so users can obtain pasteable evidence.
...
- DDR 0133b — Review progress is marked only by no-High pass evidence (amended)
