# ADR 0162: Schema report templates point to write-time authority

<!-- adr-index: supersedes 0106 -->
...
- Co-land scaffold rewrite, schema instruction pointers, and the authority test in one buildable commit boundary (ADR 0149).
...
This decision **supersedes** ADR 0106 (`docs/adr/0106-keep-both-report-template-families-pinned-parity.md`). The 0106 body remains historically accurate and unmodified. The unrelated same-number ADR `0106-run-path-baseline-predicate.md` is out of scope and unchanged.

## Alternatives Considered

- **Keep pinned parity (ADR 0106 status quo)** — rejected: non-authoritative surface drifts and doubles maintenance.
...
- **Amend ADR 0106 in place** — rejected: loses historical context and risks conflating the unrelated run-path baseline ADR sharing number 0106.
