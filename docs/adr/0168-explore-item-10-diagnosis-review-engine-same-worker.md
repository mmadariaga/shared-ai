...
<!-- adr-index: refs 0140; refs 0158; refs 0167 -->
...
When a resolved supervised spec/design worker returns `failed` (any closed class) or, under ADR 0167, `cancelled`, and the phase `diagnosis_rounds` counter is unused, Explore:
...
- ADR 0140 — Supervised rounds invoke the Review Engine in-session
- ADR 0158 — Shared non-clean-closure diagnosis lives in the command runner
- ADR 0167 — Named Explore Auto cancellation exception in Bounded Recovery
