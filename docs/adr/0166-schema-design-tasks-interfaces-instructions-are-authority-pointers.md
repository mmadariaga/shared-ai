# ADR 0166: Schema design/tasks/interfaces instructions are authority pointers

<!-- adr-index: refs 0155; refs 0162 -->
...
ADR 0155 already established the informative-pointer pattern for the `change-overview` instruction. ADR 0162 established the same pattern for report and implementation schema templates. This decision extends the pointer pattern to the three design-phase artifact instruction blocks.
...
- Pattern is consistent with ADR 0155 (change-overview) and ADR 0162 (report templates).
