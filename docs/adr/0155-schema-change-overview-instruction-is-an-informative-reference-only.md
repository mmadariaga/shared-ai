# ADR 0155: Schema change-overview instruction is an informative reference only
...
The command-owned path that the instruction now points to was established by [ADR 0146](./0146-command-owned-files-live-in-their-consuming-command-directory.md), which moved `change-overview.md` from the retired `sai/` root into `sai/commands/design/`.
