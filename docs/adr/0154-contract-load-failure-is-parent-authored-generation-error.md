# ADR 0154: Contract-load failure is parent-authored generation-error
...
- **Carry contract norms as a fourth prompt element so the generator can self-classify** — rejected; violates the minimal-prompt requirement of ADR 0152.

## Consequences

- The minimal dispatch prompt (ADR 0152) stays intact: no fourth prompt element carries contract norms.
