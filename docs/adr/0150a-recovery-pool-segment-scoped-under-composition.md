...
<!-- adr-index: amends 0139b; refs 0147a -->
...
ADR 0139b made `recovery_policy: true` an immutable invocation-level opt-in with a fixed three-attempt pool. Under multi-adapter composition, a depleted earlier segment would starve a later segment (for example chained apply) if the pool stayed invocation-scoped across the whole run.
