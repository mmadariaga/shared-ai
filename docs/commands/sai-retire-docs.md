...
Removal is never automatic. The user receives a separate proposal for each candidate and must explicitly confirm which ones should be archived. Rejected candidates remain where they are, and the command does not modify the other documents.
