...
- **npm test**: The runner prints a large `npx shared-ai uninstall` dry-run inventory listing into the output before the `node:test` summary, and the failing test's detail dump (including a huge `actual` value) is printed AFTER the summary block; the meaningful result is the `ℹ tests N`, `ℹ pass N`, `ℹ fail N` count lines, so evaluate those lines, not the physical output tail. When output is redirected to a file through PowerShell, the `ℹ` glyph mangles and patterns containing it fail — match the count lines with plain `tests \d+` / `pass \d+` / `fail \d+` instead. At this repo's current HEAD the full-suite baseline is 514 tests / 507 pass / 7 fail, all pre-existing.
  *Observed:* terminal-doc-commit-session-auth — the fresh pre-edit baseline and the post-edit full-suite comparison both measured 514/507/7 at HEAD, superseding the previously recorded 474/466/8 (an unrelated commit landing between planning and apply shifted the absolute counts).
